...
async def addconnection(client, message):
    ...
    if message.chat.type == enums.ChatType.PRIVATE:
        if len(message.command) < 2:
            return await message.reply_text("⚙️ **Usage:** `/connect <group_id>`")
    ...
    try:
        ...
        await message.reply_text(
            "❌ Invalid Group ID or I am not present in the group."
        )
...
async def connections(client, message):
    ...
    if not groupids:
        return await message.reply_text("⚠️ No active connections!")
    ...
    for gid in groupids:
        try:
            ...
            active = (
                " - ACTIVE"
                if await if_active(str(message.from_user.id), str(gid))
                else ""
            )
            buttons.append(
                [
                    InlineKeyboardButton(
                        text=f"{chat.title}{active}",
                        callback_data=f"groupcb:{gid}:{active}",
                    )
                ]
            )
        except Exception:
    ...
    if buttons:
        await message.reply_text(
            "🔗 **Your Connected Groups:**", reply_markup=InlineKeyboardMarkup(buttons)
        )
    else:
        await message.reply_text("⚠️ No active connections!")
