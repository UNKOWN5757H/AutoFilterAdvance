...
try:
    ...
    db_client = (
        AsyncIOMotorClient(info.DATABASE_URI)
        if getattr(info, "DATABASE_URI", None)
        else None
    )
...
async def generate_backup_file() -> str:
    ...
    backup_data = {
        coll: await bot_db[coll].find({}).to_list(length=None) for coll in collections
    }
    ...
    while True:
        ...
        try:
            ...
            await bot.send_document(
                ...
                document=file_path,
                caption=f"🔄 **Automated Daily Backup**\n📅 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            )
            os.remove(file_path)
...
async def db_backup_cmd(bot: Client, message: Message):
    if not bot_db:
        return await message.reply_text("❌ **MongoDB missing.**")
...
async def db_restore_cmd(bot: Client, message: Message):
    ...
    if (
        ...
        or not message.reply_to_message.document.file_name.endswith(".json")
    ):
