...
class ForceAddDB:
    def __init__(self, filepath="force_add_data.json"):
        ...
        self.user_adds = {}  # chat_id_user_id -> {"t": total_adds, "h": [timestamp1, timestamp2]}
        self.new_users = {}  # chat_id -> [list of user_ids who joined after limit was set]
        self._load()
        ...
        if os.path.exists(self.filepath):
            try:
                with open(self.filepath, "r") as f:
                    ...
                    if "limits" in data:
                        self.settings = {int(k): {"limit": v, "mode": "all"} for k, v in data.get("limits", {}).items()}
                        self.user_adds = {k: {"t": v, "h": []} for k, v in data.get("adds", {}).items()}
                    else:
                        self.settings = {int(k): v for k, v in data.get("settings", {}).items()}
                        self.user_adds = data.get("adds", {})
                        self.new_users = {int(k): v for k, v in data.get("new_users", {}).items()}
    ...
    def increment_adds(self, chat_id: int, user_id: int, count: int):
        ...
        week_ago = now - 604800
        self.user_adds[key]["h"] = [ts for ts in self.user_adds[key]["h"] if ts >= week_ago]
        self._save()
...
async def set_force_add(bot: Client, message: Message):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("👥 Force Add for ALL Members", callback_data=f"fa_set_{limit}_all_{admin_id}")],
        [InlineKeyboardButton("🆕 Force Add for ONLY NEW Members", callback_data=f"fa_set_{limit}_new_{admin_id}")]
    ])
