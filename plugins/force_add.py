...
async def is_admin(bot: Client, chat_id: int, user_id: int) -> bool:
    ...
    try:
        ...
        return any(
            role in str(getattr(member, "status", "")).lower()
            for role in ["administrator", "creator", "owner"]
        )
    except Exception:
        return False
        ...
        [
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"fa_set_{limit}_all_{message.from_user.id}",
                )
            ],
            [
                InlineKeyboardButton(
                    "🆕 Force Add for ONLY NEW Members",
                    callback_data=f"fa_set_{limit}_new_{message.from_user.id}",
                )
            ],
        ]
    )
    await message.reply_text(
        "🎯 **Who should this requirement apply to?**", reply_markup=kb
    )
...
@Client.on_message(
    filters.command(["topaddall", "topadd24", "topadd7"]) & filters.group
)
...
async def enforce_force_add(bot: Client, message: Message):
    ...
    if settings.get("mode") == "new" and not await plugin_db.is_fa_new_user(
            ...
            warn_msg = await message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "📊 Check My Progress", callback_data="forceadd_check"
                            )
                        ]
                    ]
                ),
...
async def check_adds_button(bot: Client, query):
    ...
    current = await plugin_db.get_fa_user_adds(
    ...
    if current >= limit:
        await query.answer(
            f"✅ You have added {current} members. Chat freely!", show_alert=True
        )
    else:
        await query.answer(
            f"⚠️ Added {current}/{limit} members.\nYou need {limit - current} more to chat.",
            show_alert=True,
        )
