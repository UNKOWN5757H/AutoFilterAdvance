...
async def _tmdb_get(path, params=None):
    ...
    if TMDB_API_KEY: _params["api_key"] = TMDB_API_KEY
    elif TMDB_BEARER_TOKEN: _headers = {"Authorization": f"Bearer {TMDB_BEARER_TOKEN}", "Content-Type": "application/json;charset=utf-8"}
...
async def _search_media_id(query: str):
    ...
    result = await _tmdb_get("search/multi", {"query": title, "language": "en-US", "page": 1, "include_adult": "false"})
    multi_results = result.get("results", [])
    if not multi_results: return None, None

    def get_ratio(s1, s2): return SequenceMatcher(None, (s1 or "").lower(), (s2 or "").lower()).ratio()
    scored = [(r, get_ratio(r.get("title") or r.get("name"), title)) for r in multi_results]
    scored.sort(key=lambda x: x[1], reverse=True)
    
    for r, _ in scored:
        if r.get("media_type") in ["movie", "tv"]: return r["media_type"], r["id"]
    return None, None
...
async def _fetch_tmdb_data(query: str):
    ...
    details = await _tmdb_get(f"{media_type}/{media_id}", {"append_to_response": "credits,external_ids,images"})
    ...
    return {
        ...
        "languages": ", ".join(l["english_name"] for l in details.get("spoken_languages", [])),
        "director": ", ".join(c["name"] for c in crew if c.get("job") == "Director"),
        "cast": ", ".join(c["name"] for c in details.get("credits", {}).get("cast", [])[:15]),
        "plot": details.get("overview"),
        "poster_url": f"{TMDB_IMAGE_BASE_URL}{details.get('poster_path')}" if details.get("poster_path") else None,
        "backdrop_url": f"{TMDB_IMAGE_BASE_URL}{details.get('backdrop_path')}" if details.get("backdrop_path") else None,
...
async def get_movie_detailsx(query, id=False, file=None):
    ...
    for key in ("genres", "languages", "countries", "director", "writer", "producer", "composer", "cinematographer", "cast"):
        if isinstance(data.get(key), str): data[key] = [s.strip() for s in data[key].split(",") if s.strip()]
    return data
