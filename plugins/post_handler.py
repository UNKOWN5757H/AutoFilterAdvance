...
async def _build_final_post_content(session: dict, session_id: int):
    ...
    # Safely format and append extra elements if they aren't explicitly inside the template
    if (
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "✏️ Edit Buttons",
                            callback_data=f"post:edit_buttons:{session_id}",
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            "❌ Cancel Post", callback_data=f"post:cancel:{session_id}"
                        )
                    ],
                ]
            ),
