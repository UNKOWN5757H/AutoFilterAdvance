...
import requests  # Required for the multi-host uploader
...
# ============================================================
# 🌐 TRIPLE-FAIL-SAFE IMAGE UPLOADER
# ============================================================
def _upload_sync(file_path):
    """Synchronous triple-fallback upload to bypass server IP blocks."""
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36"
    }
    
    # 1. Try Telegraph (Primary)
    try:
        with open(file_path, "rb") as f:
            res = requests.post("https://telegra.ph/upload", files={'file': ('img.jpg', f, 'image/jpeg')}, headers=headers, timeout=10)
    ...
    # 2. Try Graph.org (Telegram Backup)
    try:
        with open(file_path, "rb") as f:
            res = requests.post("https://graph.org/upload", files={'file': ('img.jpg', f, 'image/jpeg')}, headers=headers, timeout=10)
        if res.status_code == 200:
            data = res.json()
            if isinstance(data, list) and "src" in data[0]:
    ...
    # 3. Try Catbox (Bulletproof Fallback)
    try:
        with open(file_path, "rb") as f:
            payload = {"reqtype": "fileupload"}
            res = requests.post("https://catbox.moe/user/api.php", data=payload, files={"fileToUpload": ('img.jpg', f, 'image/jpeg')}, headers=headers, timeout=15)
        if res.status_code == 200 and res.text.startswith("http"):
            return res.text.strip()
    except Exception as e:
        logger.error(f"All image uploads failed: {e}")
...
async def upload_image_safely(client: Client, message: Message):
    try:
        ...
        # Run the synchronous multi-host uploader in a background thread
        url = await asyncio.to_thread(_upload_sync, file_path)
        ...
        logger.error(f"Image Download/Processing Error: {e}")
    return None
...
async def edit_image_cmd(client: Client, message: Message):
    ...
    try:
        ...
        if response.photo:
            ...
            url = await upload_image_safely(client, response)
            if url:
                session["custom_poster"] = url
                await status_msg.edit_text("✅ Image successfully uploaded and set as rich preview!")
            else:
                await status_msg.edit_text("❌ All uploads failed (Servers blocked the IP). Please provide a URL instead.")
                return
        elif response.text:
            if response.text.strip() == "/reset":
                ...
                return await message.reply_text("⚠️ Invalid input. Must be a photo, a URL, or /reset.")
...
async def update_post_preview(client: Client, session_id: int, chat_id: int, force_resend: bool = False):
    ...
    try:
        # Telegraph invisible link mode always enabled for rich text previews
...
async def handle_set_poster(client: Client, query: CallbackQuery, session_id: int):
    session = post_sessions[session_id]
    response = await get_user_input(client, query, session, "📸 Send a photo or an image URL.\n*(Or type `/reset` to use the default poster)*")
    if response:
        if response.photo:
            try:
                await query.message.edit_text("⏳ Uploading to secure image host for preview...")
            ...
            if url:
                ...
                try:
                    await query.message.edit_text("❌ All uploads failed (Servers blocked the IP). Please provide a URL instead.")
                except Exception: pass
...
async def finalize_and_post(client: Client, query: CallbackQuery, session_id: int, _=None):
    ...
    try:
        # Send Telegraph Rich Preview Text format unconditionally 
        text_content = f"{final_caption}\n<a href='{poster_to_use}'>&#8205;</a>" if poster_to_use else final_caption
