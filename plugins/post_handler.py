...
# ============================================================
# 🌐 TELEGRAPH UPLOADER (Converts images to rich web URLs)
# ============================================================
async def upload_to_telegraph(client: Client, message: Message):
    try:
        ...
        form.add_field('file', file_path.getvalue(), filename='image.jpg', content_type='image/jpeg')
...
async def edit_image_cmd(client: Client, message: Message):
    ...
    # Ask the user for the image interactively
    ...
    try:
        ...
        if response.photo:
            status_msg = await message.reply_text("⏳ Uploading to Telegraph for rich preview...")
            url = await upload_to_telegraph(client, response)
            if url:
                session["custom_poster"] = url
                await status_msg.edit_text("✅ Image successfully uploaded and set as Telegraph preview!")
            else:
                await status_msg.edit_text("❌ Failed to upload image to Telegraph. Please try a URL instead.")
                return
        elif response.text:
...
async def update_post_preview(client: Client, session_id: int, chat_id: int, force_resend: bool = False):
    ...
    try:
        # ⚡ ALWAYS generate a rich text message with an invisible link at the very end to create a large preview
...
def build_keyboard(session: dict, session_id: int):
    ...
    rows.extend(
        [
            ...
            [
                # Removed "Photo Mode" toggle entirely; Telegraph preview is now standard
                InlineKeyboardButton(
...
async def handle_set_poster(client: Client, query: CallbackQuery, session_id: int):
    ...
    if response:
        if response.photo:
            try:
                await query.message.edit_text("⏳ Uploading to Telegraph for rich preview...")
            ...
            if url:
                session["custom_poster"] = url
                try:
                    await query.answer("Image set as Telegraph preview!", show_alert=True)
                except Exception: pass
...
async def finalize_and_post(client: Client, query: CallbackQuery, session_id: int, _=None):
    ...
    try:
        # ALWAYS send as Text + URL Preview (No send_photo!)
        text_content = f"{final_caption}\n<a href='{poster_to_use}'>&#8205;</a>" if poster_to_use else final_caption
