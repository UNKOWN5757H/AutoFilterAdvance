...
# ============================================================
# 🌐 TELEGRAPH UPLOADER (Fixed for strict API rejection)
# ============================================================
async def upload_to_telegraph(client: Client, message: Message):
    try:
        # Download to hard disk instead of RAM to prevent byte-stream errors
        file_path = await client.download_media(message)
        if not file_path or not os.path.exists(file_path):
            return None

        # Add Google Chrome headers so Telegraph accepts the request
