...
# ============================================================
# 🌐 4-LAYER IN-MEMORY IMAGE UPLOADER (NO DISK I/O REQUIRED)
# ============================================================
def _upload_sync(file_bytes):
    """Synchronous 4-layer fallback upload to bypass server IP blocks and read-only disks."""
    headers = {
    ...
    # 2. Try Envs.sh (Bulletproof CLI pastebin)
    try:
    ...
    # 3. Try Catbox (High capacity fallback)
    try:
...
async def upload_image_safely(client: Client, message: Message):
    """Downloads image directly to RAM to bypass read-only filesystems, then uploads it."""
    try:
        # Download directly into memory (BytesIO) instead of physical disk
        ...
        if not url:
            return None, "❌ All upload servers (Telegraph, Envs, Catbox) rejected the connection. Server IP is completely blacklisted."
