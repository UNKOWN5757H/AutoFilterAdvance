...
async def edit_post_cmd(client: Client, message: Message):
    try:
        ...
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("Normal Img", callback_data=f"post:editpost_mode:{user_id}:normal"),
                InlineKeyboardButton("Preview Img", callback_data=f"post:editpost_mode:{user_id}:preview")
            ],
            [
                InlineKeyboardButton("IMDB Img", callback_data=f"post:editpost_mode:{user_id}:imdb")
            ]
        ])

        status_msg = await status_msg.edit_text("🖼️ **Select Image Mode for this Edit Session:**", reply_markup=keyboard)
...
async def post_callbacks(client: Client, query: CallbackQuery):
    try:
        ...
        if action == "back":
            ...
            mode = extra_data[0]
            if mode == "normal": session["photo_mode"] = True
            elif mode == "preview": session["photo_mode"] = False
            elif mode == "imdb":
                session["custom_poster"] = None
                session["photo_mode"] = False
...
async def handle_set_poster(client: Client, query: CallbackQuery, session_id: int):
    ...
    if response:
        if response.photo:
            try: await query.message.edit_text("⏳ Uploading image for preview...")
            except Exception: pass
            ...
            if url:
                session["custom_poster"] = url
                session["photo_mode"] = False
                try: await query.answer("Image set as rich preview!", show_alert=True)
                except Exception: pass
            else:
                try: await query.message.edit_text(err)
                except Exception: pass
                
        elif response.text:
            text_input = response.text.strip().lower()
            if text_input == "/reset": session["custom_poster"] = None
