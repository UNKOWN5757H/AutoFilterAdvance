...
def _upload_sync(file_bytes):
    ...
    try:
        res = requests.post(
            ...
            files={"files[]": ("img.jpg", file_bytes, "image/jpeg")},
            headers=headers,
            timeout=15,
        )
        if res.status_code == 200:
    ...
    if len(message.command) > 1:
        ...
        try:
            response = await client.listen(
                chat_id=message.chat.id, user_id=user_id, timeout=120
            )
...
@Client.on_message(
    filters.command(["Changebuttoncolour", "changebuttoncolour"]) & admin_filter,
    group=-4,
)
async def change_button_colour_cmd(client: Client, message: Message):
    ...
    if not found:
        return await message.reply_text(
            f"❌ Button number {btn_num} not found. You only have {count} buttons in the current layout."
        )
...
async def _build_final_post_content(session: dict, session_id: int):
    ...
    if c_langs == ["BLANK"]:
        ...
        langs_str = ", ".join(c_langs) if c_langs else "N/A"
    ...
    if c_res == ["BLANK"]:
        ...
        res_str = ", ".join(c_res) if c_res else "N/A"
    ...
    if c_gen == ["BLANK"]:
        ...
        genres_str = ", ".join(c_gen) if c_gen else "N/A"
    ...
    if c_ott == ["BLANK"]:
        ...
        otts_str = ", ".join(c_ott) if c_ott else "N/A"
    ...
    if (
        ...
        and session["custom_languages"] != ["BLANK"]
    ...
    if (
        ...
        and session["custom_resolutions"] != ["BLANK"]
    ...
    if (
        ...
        and session["custom_genres"] != ["BLANK"]
