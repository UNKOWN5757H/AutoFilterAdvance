...
async def edit_normal_image_cmd(client: Client, message: Message):
    ...
    try:
        ...
        if response.photo:
            if session.get("edit_target") and not session.get("photo_mode"):
                return await message.reply_text(
                    "❌ This imported post is in Preview Mode. You cannot use a raw Telegram file. Please send a URL or use /editimage instead."
                )
...
async def _build_final_post_content(session: dict, session_id: int):
    ...
    clean_title = (
        ...
        else html.escape(str(title_val if title_val else "N/A"))
        .replace("{", "(")
        .replace("}", ")")
        .replace("[", "")
        .replace("]", "")
    ...
    clean_year = (
        ...
        else html.escape(str(year_val if year_val else "N/A"))
        .replace("{", "(")
        .replace("}", ")")
        .replace("[", "")
        .replace("]", "")
