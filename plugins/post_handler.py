...
# ============================================================
# 🔄 EDIT EXISTING POST COMMAND & MENU PROMPT
# ============================================================
...
async def edit_post_cmd(client: Client, message: Message):
    try:
        ...
        # Analyze existing message structure
        if target_msg.photo:
        ...
        post_sessions[user_id] = {
            ...
            "buttons": buttons,
            "photo_mode": bool(target_msg.photo),
            "use_landscape": False,
            "custom_languages": [],
            "custom_resolutions": [],
            "custom_genres": [],
        ...
        # Prompt user to choose Image Setup
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "Normal Img",
                        callback_data=f"post:editpost_mode:{user_id}:normal",
                    ),
                    InlineKeyboardButton(
                        "Preview Img",
                        callback_data=f"post:editpost_mode:{user_id}:preview",
                    ),
                ],
                [
                    InlineKeyboardButton(
                        "IMDB Img", callback_data=f"post:editpost_mode:{user_id}:imdb"
                    )
                ],
            ]
        )
        ...
        status_msg = await status_msg.edit_text(
            "🖼️ **Select Image Mode for this Edit Session:**", reply_markup=keyboard
        )
        post_sessions[user_id]["last_preview_message_id"] = status_msg.id
...
async def _build_final_post_content(session: dict, session_id: int):
    ...
    if str(title_val).upper() == "BLANK":
        ...
        clean_title = (
            html.escape(str(title_val if title_val else "N/A"))
            .replace("{", "(")
            .replace("}", ")")
            .replace("[", "")
            .replace("]", "")
    ...
    if str(year_val).upper() == "BLANK":
        ...
        clean_year = (
            html.escape(str(year_val if year_val else "N/A"))
            .replace("{", "(")
            .replace("}", ")")
            .replace("[", "")
            .replace("]", "")
    ...
    rating_str = (
        html.escape(
            str(movie_details.get("rating") if movie_details.get("rating") else "N/A")
        )
        .replace("{", "(")
        .replace("}", ")")
    )
    plot_str = (
        html.escape(
            str(movie_details.get("plot") if movie_details.get("plot") else "N/A")
        )
        .replace("{", "(")
        .replace("}", ")")
    )
        ...
        if is_normal_photo:
            if force_resend:
                ...
                try:
                    ...
                    # Switch fails if editing text to media, force resend
                    await update_post_preview(
            ...
            if force_resend:
                ...
                try:
                    ...
                    # Switch fails if editing media to text, force resend
                    await update_post_preview(
                        client, session_id, chat_id, force_resend=True
                    )
                    return
...
async def post_callbacks(client: Client, query: CallbackQuery):
    try:
        ...
        if action == "back":
            ...
            session["last_preview_message_id"] = None
            await update_post_preview(
                client, session_id, query.message.chat.id, force_resend=True
            )
            return
