        ...
        with open(file_path, "rb") as f:
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                async with session.post(
                    "https://telegra.ph/upload", data=form
                ) as response:
    ...
    try:
        ...
        if response.photo:
            status_msg = await message.reply_text(
                "⏳ Uploading to Telegraph for rich preview..."
            )
            url = await upload_to_telegraph(client, response)
            if url:
                session["custom_poster"] = url
                await status_msg.edit_text(
                    "✅ Image successfully uploaded and set as Telegraph preview!"
                )
            else:
                await status_msg.edit_text(
                    "❌ Failed to upload image to Telegraph. Please try a URL instead."
                )
                return
        elif response.text:
    ...
    if response:
        if response.photo:
            ...
            if url:
                ...
                try:
                    await query.answer(
                        "Image set as Telegraph preview!", show_alert=True
                    )
                except Exception:
                    pass
