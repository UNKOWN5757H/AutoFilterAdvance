...
def _upload_sync(file_bytes):
    """Synchronous fallback upload chain."""
    headers = {
...
async def edit_post_cmd(client: Client, message: Message):
    try:
        ...
        html_text = ""
        if target_msg.text:
            html_text = target_msg.text.html
        elif target_msg.caption:
            html_text = target_msg.caption.html
...
async def edit_button_colour_cmd(client: Client, message: Message):
    ...
    if len(message.command) < 3:
        return await message.reply_text(
            "❌ **Usage:** `/editbuttoncolour <button_number> <colour>`\n\n"
