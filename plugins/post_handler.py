...
LANGUAGES = ["Kannada", "English", "Hindi", "Malayalam", "Tamil", "Telugu", "#NotAvailable"]
RESOLUTIONS = ["480p", "720p", "1080p", "2160p", "4K", "WEB-DL", "HDRip", "HEVC", "#NotAvailable"]
GENRES = ["Action", "Adventure", "Animation", "Comedy", "Crime", "Drama", "Fantasy", "Horror", "Romance", "Sci-Fi", "Thriller", "#NotAvailable"]
OTT_PLATFORMS = ["Aha", "JioHotstar", "JioCinema", "SonyLIV", "Zee5", "Amazon Prime Video", "Netflix", "NotAvailable"]
...
async def post_command(client: Client, message: Message):
    ...
    post_sessions[message.from_user.id] = {
        ...
        "watermark": DEFAULT_WATERMARK, "lang_format": "<b>🔊 : {langs}</b>", "ott_format": "\n<b>📺 : #{otts}</b>",
...
async def _build_final_post_content(session: dict, session_id: int):
    ...
    if not session.get("is_manual_caption"):
        format_args = SafeDict(title=str(movie_details.get("title", "N/A")), year=str(movie_details.get("year", "N/A")), rating=str(movie_details.get("rating", "N/A")), plot=str(movie_details.get("plot", "N/A")), LANGUAGES=langs_str, RESOLUTIONS=res_str, GENRES=genres_str, OTT_PLATFORMS=otts_str, langs=langs_str, resolutions=res_str, genres=genres_str, otts=otts_str)
...
async def update_post_preview(client: Client, session_id: int, chat_id: int, force_resend: bool = False):
    ...
    if not session["last_preview_message_id"] or force_resend:
        ...
        if session["photo_mode"] and poster_to_use:
            sent_message = await client.send_photo(chat_id, photo=poster_to_use, caption=final_caption, reply_markup=keyboard)
        else:
            sent_message = await client.send_message(chat_id, f"<a href='{poster_to_use}'>&#8205;</a>{final_caption}" if poster_to_use else final_caption, reply_markup=keyboard)
...
async def post_callbacks(client: Client, query: CallbackQuery):
    ...
    if action in ["languages", "resolutions", "genres", "otts"]:
        ...
        item = extra_data[0]
        t_map = {"select_lang": ("custom_languages", "languages"), "select_res": ("custom_resolutions", "resolutions"), "select_gen": ("custom_genres", "genres"), "select_ott": ("custom_otts", "otts")}
        lst, ret = t_map[action]
        if item in session[lst]: session[lst].remove(item)
        else: session[lst].append(item)
        return await show_selection_menu(query, session_id, ret)

    elif action == "edit_buttons":
        ask_msg = await query.message.reply_text("Send the button layout. Format:\n`Btn 1 - URL1 | Btn 2 - URL2`")
        try:
            response = await client.listen(chat_id=query.message.chat.id, filters=filters.user(query.from_user.id), timeout=60)
            new_layout = []
            for row_str in response.text.strip().split("\n"):
                ...
                for btn_str in row_str.split("|"):
                    if " - " in btn_str:
                        ...
                        row_btns.append(InlineKeyboardButton(text.strip(), url=url.strip()))
                if row_btns: new_layout.append(row_btns)
            session["buttons"] = new_layout
            await response.delete(); await ask_msg.delete()
        except asyncio.TimeoutError: await ask_msg.delete()
        ...
        final_keyboard = InlineKeyboardMarkup(session["buttons"]) if session["buttons"] else None
        
        try: await client.delete_messages(query.message.chat.id, session["last_preview_message_id"])
        except Exception: pass
        
        if session["photo_mode"] and poster_to_use:
            await client.send_photo(MOVIE_UPDATE_CHANNEL, photo=poster_to_use, caption=final_caption, reply_markup=final_keyboard)
        else:
            await client.send_message(MOVIE_UPDATE_CHANNEL, text=f"<a href='{poster_to_use}'>&#8205;</a>{final_caption}" if poster_to_use else final_caption, reply_markup=final_keyboard)
...
async def show_selection_menu(query: CallbackQuery, session_id: int, menu_type: str):
    ...
    m_map = {"languages": (LANGUAGES, "custom_languages", "select_lang"), "resolutions": (RESOLUTIONS, "custom_resolutions", "select_res"), "genres": (GENRES, "custom_genres", "select_gen"), "otts": (OTT_PLATFORMS, "custom_otts", "select_ott")}
    items, selected, action_prefix = m_map[menu_type]
    
    buttons = [InlineKeyboardButton(f"✅ {i}" if i in session[selected] else i, callback_data=f"post:{action_prefix}:{session_id}:{i}") for i in items]
