...
DEFAULT_WATERMARK = "<a href='https://t.me/sandalwood_kannada_moviesz'>Join Sandalwood New Movies</a>"
...
OTT_PLATFORMS = ["Aha", "ALTBalaji", "JioHotstar", "ErosNow", "Hoichoi", "JioCinema", "MXPlayer", "SonyLIV", "SunNXT", "Voot", "Zee5", "Amazon Prime Video", "Apple TV+", "Crunchyroll", "Discovery+", "HBO Max", "Hulu", "Netflix", "Paramount+", "Peacock", "YouTube Premium", "NotAvailable"]
...
# ⚡ FIXED: Removed group=-4 to prevent routing collisions with auto_filter
@Client.on_message(filters.command("post") & admin_filter)
async def post_command(client: Client, message: Message):
    try:
        ...
        await message.reply_text(f"❌ **CRITICAL COMMAND ERROR:**\n\n```python\n{traceback.format_exc()[:4000]}\n```")
...
async def start_post_session(client: Client, message: Message, user_id: int, movie_name: str):
    ...
    try:
        ...
        await message.reply_text(f"❌ **SESSION START ERROR:**\n\n```python\n{traceback.format_exc()[:4000]}\n```")
...
async def _build_final_post_content(session: dict, session_id: int):
    ...
    rating_str = html.escape(str(movie_details.get("rating", "N/A"))).replace("{", "(").replace("}", ")")
    plot_str = html.escape(str(movie_details.get("plot", "N/A"))).replace("{", "(").replace("}", ")")
    clean_title = html.escape(str(movie_details.get("title", "N/A"))).replace("{", "(").replace("}", ")").replace("[", "").replace("]", "")
    clean_year = html.escape(str(movie_details.get("year", "N/A"))).replace("{", "(").replace("}", ")").replace("[", "").replace("]", "")
...
async def update_post_preview(client: Client, session_id: int, chat_id: int, force_resend: bool = False):
    ...
    if not final_caption:
        try: return await client.edit_message_text(chat_id, session["last_preview_message_id"], "Could not format details for this movie.")
        except Exception: return
    ...
    try:
        if session["photo_mode"] and poster_to_use:
            ...
            if force_resend:
                old_msg_id = session.get("last_preview_message_id")
                sent_message = await client.send_message(chat_id, text_content, reply_markup=keyboard, reply_to_message_id=session["original_message_id"])
                session["last_preview_message_id"] = sent_message.id
                if old_msg_id:
                    try: await client.delete_messages(chat_id, old_msg_id)
                    except Exception: pass
            else:
                await client.edit_message_text(chat_id, session["last_preview_message_id"], text_content, reply_markup=keyboard)
    except Exception:
...
def build_keyboard(session: dict, session_id: int):
    ...
    rows.extend([
        ...
        [InlineKeyboardButton(f"Mode: {'Photo' if session['photo_mode'] else 'Text'}", callback_data=f"post:toggle_preview:{session_id}"), InlineKeyboardButton(f"Poster: {'Landscape' if session['use_landscape'] else 'Portrait'}", callback_data=f"post:toggle_poster:{session_id}")],
...
async def post_callbacks(client: Client, query: CallbackQuery):
    try:
        ...
        if action == "back":
            ...
            # ⚡ FIXED: Await statements were missing here causing immediate silence crashes
            ...
            if action == "edit_buttons":
                ...
                # ⚡ FIXED: Await statement missing here
        ...
        await query.message.reply_text(f"❌ **CALLBACK CRITICAL ERROR:**\n\n```python\n{traceback.format_exc()[:4000]}\n```")
...
async def handle_add_get_files(client: Client, session: dict) -> bool:
    ...
    if movie_details:
        ...
        session["buttons"].append([InlineKeyboardButton(text="Direct Search", url=url)])
        return True
    return False
    ...
    if response:
        if response.photo:
            ...
            if not session["photo_mode"]:
                ...
                try: await query.answer("Switched to Photo mode as you uploaded an image.", show_alert=True)
                except Exception: pass
        elif response.text and response.text.startswith("http"): session["custom_poster"] = response.text
        elif response.text and response.text == "/reset": session["custom_poster"] = None
    return True
...
async def handle_toggle_preview(query: CallbackQuery, session: dict):
    if session.get("custom_poster") and not session["custom_poster"].startswith("http"):
        try: await query.answer("Cannot switch to Text mode with an uploaded photo.", show_alert=True)
        except Exception: pass
        return False
...
async def finalize_and_post(client: Client, query: CallbackQuery, session_id: int, _=None):
    ...
    try:
        if mode == "Photo":
            await client.send_photo(chat_id=MOVIE_UPDATE_CHANNEL, photo=poster_to_use, caption=final_caption, reply_markup=final_keyboard)
        else:
            text_content = f"<a href='{poster_to_use}'>&#8205;</a>{final_caption}" if poster_to_use else final_caption
