...
LANGUAGES = ["Kannada", "English", "Gujarati", "Hindi", "Bengali", "Malayalam", "Marathi", "Punjabi", "Tamil", "Telugu", "Urdu", "Arabic", "French", "German", "Italian", "Japanese", "Korean", "Mandarin", "Portuguese", "Russian", "Spanish", "#NotAvailable"]
RESOLUTIONS = ["144p", "240p", "480p", "720p", "1080p", "1440p", "2160p", "4320p", "BluRay", "BDRip", "WEB-DL", "HDRip", "WEBRip", "HDTVRip", "DVDRip", "DVDScr", "TSRip", "CAMRip", "HDTC", "HEVC", "#NotAvailable"]
GENRES = ["Action", "Adventure", "Animation", "Biography", "Comedy", "Crime", "Documentary", "Drama", "Family", "Fantasy", "History", "Horror", "Music", "Musical", "Mystery", "Romance", "Sci-Fi", "Sport", "Thriller", "War", "Western", "Superhero", "Psychological", "Suspense", "Noir", "Disaster", "Survival", "Teen", "Slice of Life", "Coming of Age", "Martial Arts", "Political", "Legal", "Medical", "Spy", "Erotic", "Mythology", "Short", "Experimental", "#NotAvailable"]
OTT_PLATFORMS = ["Aha", "ALTBalaji", "JioHotstar", "ErosNow", "Hoichoi", "JioCinema", "MXPlayer", "SonyLIV", "SunNXT", "Voot", "Zee5", "AmazonPrime", "AppleTV+", "Crunchyroll", "Discovery+", "HBO Max", "Hulu", "Netflix", "Paramount+", "Peacock", "ManoramaMAX", "NotAvailable"]
...
def _upload_sync(file_bytes):
    ...
    try:
        ...
        if res.status_code == 200:
            data = res.json()
            if isinstance(data, list) and "src" in data[0]: return "http://telegraph.controller.bot" + data[0]["src"]
            elif isinstance(data, dict) and "link" in data: return data["link"]
    except Exception: pass
    try:
        res = requests.post("https://telegra.ph/upload", files={"file": ("img.jpg", file_bytes, "image/jpeg")}, headers=headers, timeout=10)
        if res.status_code == 200:
            data = res.json()
            if isinstance(data, list) and "src" in data[0]: return "https://telegra.ph" + data[0]["src"]
    except Exception: pass
    try:
...
async def post_command(client: Client, message: Message):
    try:
        if len(message.command) == 1: return await message.reply_text("Please provide a movie name. Usage: `/post The Dark Knight`")
        movie_name = " ".join(message.command[1:])
        user_id = message.from_user.id
        await start_post_session(client, message, user_id, movie_name)
    except Exception as e: await message.reply_text(f"❌ **COMMAND ERROR:**\n`{e}`")


@Client.on_message(filters.command("editpost") & admin_filter, group=-4)
async def edit_post_cmd(client: Client, message: Message):
    try:
        if len(message.command) < 2: return await message.reply_text("❌ Please provide a post link.\n**Usage:** `/editpost https://t.me/c/1923564465/1410`")
        ...
        try:
            ...
            if not target_msg or target_msg.empty: return await status_msg.edit_text("❌ Message not found. Make sure the bot is an admin in the channel.")
        except Exception as e: return await status_msg.edit_text(f"❌ Could not fetch message:\n`{e}`")
        ...
        if target_msg.reply_markup and target_msg.reply_markup.inline_keyboard:
            for row in target_msg.reply_markup.inline_keyboard:
                new_row = []
                for btn in row: new_row.append(InlineKeyboardButton(text=btn.text, url=btn.url, callback_data=btn.callback_data))
        ...
        if title_match:
            ...
            if title_match.group(2): movie_details["year"] = title_match.group(2).strip()
            l_match = re.search(r"🔊\s*:\s*(.*?)</[bB]>", html_text)
            if l_match and l_match.group(1).strip() != "N/A": c_langs = [x.strip() for x in l_match.group(1).split(",")]
            r_match = re.search(r"🖥️\s*:\s*(.*?)</[bB]>", html_text)
            if r_match and r_match.group(1).strip() != "N/A": c_res = [x.strip() for x in r_match.group(1).split(",")]
            g_match = re.search(r"🎥\s*:\s*(.*?)</[bB]>", html_text)
            if g_match and g_match.group(1).strip() != "N/A": c_gens = [x.strip() for x in g_match.group(1).split(",")]
            o_match = re.search(r"📺\s*:\s*#?(.*?)</[bB]>", html_text)
            if o_match and o_match.group(1).strip() != "N/A": c_otts = [x.strip() for x in o_match.group(1).split(",")]
            w_match = re.search(r"(<b>Jᴏɪɴ:.*?</b>)", html_text)
            if w_match: watermark = w_match.group(1)
        ...
        post_sessions[user_id] = {
            "movie_name": "Edit Session", "caption": html_text, "is_manual_caption": is_manual, "buttons": buttons, "photo_mode": is_photo_mode, "use_landscape": False, "custom_languages": c_langs, "custom_resolutions": c_res, "custom_genres": c_gens, "custom_otts": c_otts, "last_preview_message_id": status_msg.id, "original_message_id": message.id, "custom_poster": poster_url, "watermark": watermark, "lang_format": LANGUAGES_FORMAT, "ott_format": OTT_FORMAT, "gen_format": GENRES_FORMAT, "res_format": RESOLUTIONS_FORMAT, "active_template": "clean_grid", "movie_details": movie_details, "edit_target": {"chat_id": chat_id, "message_id": msg_id},
        }

        await update_post_preview(client, user_id, message.chat.id, force_resend=True)

    except Exception as e: await message.reply_text(f"❌ **EDIT POST ERROR:**\n`{e}`")
...
async def edit_title_cmd(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        ...
        except asyncio.TimeoutError: await ask_msg.edit_text("⌛ Timeout. Title edit cancelled.")
...
async def edit_year_cmd(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        ...
        except asyncio.TimeoutError: await ask_msg.edit_text("⌛ Timeout. Year edit cancelled.")
...
async def edit_button_colour_cmd(client: Client, message: Message):
    user_id = message.from_user.id
    if user_id not in post_sessions: return await message.reply_text("❌ No active post session.")
    if len(message.command) < 3: return await message.reply_text("❌ **Usage:** `/editbuttoncolour <button_number> <colour>`\n\n**Example:** `/editbuttoncolour 1 green`\n**Colours:** `green`, `red`, `blue`")
...
async def edit_direct_url(client: Client, message: Message):
    ...
    if len(message.command) > 1: new_url = message.text.split(None, 1)[1].strip()
    else:
        ...
        except asyncio.TimeoutError: return await ask_msg.edit_text("⌛ Timeout. URL edit cancelled.")

    button_updated = False
    if new_url.lower() == "blank":
        if session.get("buttons"):
            for row in session["buttons"]: row[:] = [btn for btn in row if btn.text != "Direct Search 🔎"]
            ...
            for row in session["buttons"]:
                for btn in row:
                    if btn.text == "Direct Search 🔎":
                        btn.url = new_url
                        button_updated = True
        if button_updated: await message.reply_text(f"✅ **'Direct Search' button URL updated successfully!**\n\n🔗 **New URL:** `{new_url}`")
...
async def edit_langs_cmd(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        ...
        except asyncio.TimeoutError: return await ask_msg.edit_text("⌛ Timeout. Edit cancelled.")
...
async def edit_image_cmd(client: Client, message: Message):
    ...
    try:
        ...
        if response.photo:
            ...
            else: return await message.reply_text("⚠️ Invalid input. Must be a photo, a URL, `blank`, or `/reset`.")
...
async def edit_normal_image_cmd(client: Client, message: Message):
    ...
    try:
        ...
        if response.photo:
            ...
            else: return await message.reply_text("⚠️ Invalid input.")
...
async def start_post_session(client: Client, message: Message, user_id: int, movie_name: str):
    try:
        ...
        if not movie_details: return await status_msg.edit_text("❌ Could not fetch details for the movie from TMDB.")
        ...
        post_sessions[user_id] = {
            "movie_name": movie_name, "caption": None, "is_manual_caption": False, "buttons": [], "photo_mode": False,
            "use_landscape": bool(movie_details.get("backdrop_url")), "custom_languages": [], "custom_resolutions": [],
            "custom_genres": [], "custom_otts": [], "last_preview_message_id": status_msg.id, "original_message_id": message.id,
            "custom_poster": None, "watermark": DEFAULT_WATERMARK, "lang_format": LANGUAGES_FORMAT, "ott_format": OTT_FORMAT,
            "gen_format": GENRES_FORMAT, "res_format": RESOLUTIONS_FORMAT, "active_template": "clean_grid", "movie_details": movie_details,
        }

        if USE_GETFILE_BUTTON_BY_DEFAULT: await handle_add_get_files(client, post_sessions[user_id])
        await update_post_preview(client, user_id, message.chat.id, force_resend=True)
    except Exception as e: await message.reply_text(f"❌ **SESSION ERROR:**\n`{e}`")
...
async def _build_final_post_content(session: dict, session_id: int):
    ...
    clean_title = "" if str(title_val).upper() == "BLANK" else html.escape(str(title_val if title_val else "N/A")).replace("{", "(").replace("}", ")").replace("[", "").replace("]", "")

    year_val = movie_details.get("year")
    clean_year = "" if str(year_val).upper() == "BLANK" else html.escape(str(year_val if year_val else "N/A")).replace("{", "(").replace("}", ")").replace("[", "").replace("]", "")
    ...
    if not session.get("is_manual_caption"):
        format_args = SafeDict(title=clean_title, year=clean_year, rating=rating_str, plot=plot_str, LANGUAGES=langs_str, RESOLUTIONS=res_str, GENRES=genres_str, OTT_PLATFORMS=otts_str, langs=langs_str, resolutions=res_str, genres=genres_str, otts=otts_str)
    ...
    else: base_caption = session.get("caption", "")
    ...
    if session.get("watermark"): final_caption += f"\n\n{session['watermark']}"

    keyboard = build_keyboard(session, session_id)
    poster_to_use = None if session.get("custom_poster") == "BLANK" else (session.get("custom_poster") or (movie_details.get("backdrop_url") if session.get("use_landscape") else movie_details.get("poster_url")))
    ...
    try:
        ...
        if is_normal_photo:
            if force_resend:
                ...
                sent_msg = await client.send_photo(chat_id, photo=poster_to_use, caption=final_caption, reply_markup=keyboard, reply_to_message_id=session["original_message_id"])
                session["last_preview_message_id"] = sent_msg.id
                if old_msg_id:
                    try: await client.delete_messages(chat_id, old_msg_id)
                    except Exception: pass
            else:
                try: await client.edit_message_caption(chat_id, session["last_preview_message_id"], caption=final_caption, reply_markup=keyboard)
                except Exception:
            ...
            if force_resend:
                old_msg_id = session.get("last_preview_message_id")
                sent_msg = await client.send_message(chat_id, text=text_content, reply_markup=keyboard, reply_to_message_id=session["original_message_id"], disable_web_page_preview=False)
                session["last_preview_message_id"] = sent_msg.id
                if old_msg_id:
                    try: await client.delete_messages(chat_id, old_msg_id)
                    except Exception: pass
            else:
                try: await client.edit_message_text(chat_id, session["last_preview_message_id"], text=text_content, reply_markup=keyboard, disable_web_page_preview=False)
                except Exception:
                    await update_post_preview(client, session_id, chat_id, force_resend=True)
                    return
...
def build_keyboard(session: dict, session_id: int):
    rows = []
    if session.get("buttons"): rows.extend(session["buttons"])

    mode_btn = []
    if not session.get("edit_target"): mode_btn.append(InlineKeyboardButton(f"Mode: {'Normal Img' if session.get('photo_mode') else 'Preview Img'}", callback_data=f"post:toggle_mode:{session_id}"))
    mode_btn.append(InlineKeyboardButton(f"Poster: {'Landscape' if session['use_landscape'] else 'Portrait'}", callback_data=f"post:toggle_poster:{session_id}"))

    rows.extend([
        [InlineKeyboardButton("✏️ Buttons", callback_data=f"post:buttons_menu:{session_id}"), InlineKeyboardButton("✏️ Caption", callback_data=f"post:edit_caption:{session_id}")],
        [InlineKeyboardButton("🖼️ Poster", callback_data=f"post:set_poster:{session_id}"), InlineKeyboardButton("✨ Templates", callback_data=f"post:templates:{session_id}"), InlineKeyboardButton("💧 Watermark", callback_data=f"post:set_watermark:{session_id}")],
        [InlineKeyboardButton("🔊", callback_data=f"post:languages:{session_id}"), InlineKeyboardButton("🖥️", callback_data=f"post:resolutions:{session_id}"), InlineKeyboardButton("🎥", callback_data=f"post:genres:{session_id}"), InlineKeyboardButton("📺", callback_data=f"post:otts:{session_id}")],
        mode_btn,
        [InlineKeyboardButton("✅ Post", callback_data=f"post:finalize:{session_id}"), InlineKeyboardButton("❌ Cancel", callback_data=f"post:cancel:{session_id}")],
    ])
...
async def post_callbacks(client: Client, query: CallbackQuery):
    try:
        ...
        if query.from_user.id != session_id: return await query.answer("This is not for you!", show_alert=True)
        session = post_sessions.get(session_id)
        if not session:
        ...
        if action == "back": await query.answer()
        elif action in ["languages", "resolutions", "templates", "buttons_menu", "remove_buttons_menu", "genres", "otts"]:
            await query.answer()
            ...
            if action == "edit_buttons":
                ...
                return
            elif action == "select_template": await handle_select_template(session, extra_data[0])
                ...
                if not session.get("edit_target"):
                    ...
                    force_resend = True
                else: await query.answer("You cannot switch image modes on an imported channel post!", show_alert=True)
                ...
                return
            elif action == "finalize": return await finalize_and_post(client, query, session_id)
            elif action == "cancel": return await handle_cancel(client, query, session_id)

        await update_post_preview(client, session_id, query.message.chat.id, force_resend)
    except Exception as e: await query.message.reply_text(f"❌ **CALLBACK ERROR:**\n`{e}`")

async def show_selection_menu(query: CallbackQuery, session_id: int, menu_type: str):
    session = post_sessions[session_id]
    if menu_type == "languages": items, selected, action_prefix, format_action = LANGUAGES, session["custom_languages"], "select_lang", "format_lang"
    elif menu_type == "resolutions": items, selected, action_prefix, format_action = RESOLUTIONS, session["custom_resolutions"], "select_res", "format_res"
    elif menu_type == "genres": items, selected, action_prefix, format_action = GENRES, session["custom_genres"], "select_gen", "format_gen"
    elif menu_type == "otts": items, selected, action_prefix, format_action = OTT_PLATFORMS, session["custom_otts"], "select_ott", "format_ott"
    else: return
        ...
        for row_str in response.text.strip().split("\n"):
            ...
            for btn_str in row_str.split("|"):
                if " - " in btn_str:
                    ...
                    clean_url = url.strip()
                    if not clean_url.startswith(("http://", "https://", "tg://")): clean_url = "https://" + clean_url
...
async def handle_add_get_files(client: Client, session: dict) -> bool:
    ...
    if movie_details:
        ...
        session["buttons"].append([InlineKeyboardButton(text="Group 1 🎬", url="https://t.me/Sandalwood_Kannada_Group", icon_custom_emoji_id=5258096772776991776, style=ButtonStyle.PRIMARY), InlineKeyboardButton(text="Group 2 🎬", url="https://t.me/+GLsPkRgLGGszMzY1", icon_custom_emoji_id=5258096772776991776, style=ButtonStyle.PRIMARY)])
        session["buttons"].append([InlineKeyboardButton(text="Direct Search 🔎", url=url, icon_custom_emoji_id=5258503720928288433, style=ButtonStyle.SUCCESS)])
        return True
    return False
...
async def finalize_and_post(client: Client, query: CallbackQuery, session_id: int, _=None):
    ...
    try:
        ...
        if edit_target:
            if is_normal_photo: await client.edit_message_media(chat_id=edit_target["chat_id"], message_id=edit_target["message_id"], media=InputMediaPhoto(media=poster_to_use, caption=final_caption), reply_markup=final_keyboard)
            else:
                text_content = f"{final_caption}\n<a href='{poster_to_use}'>&#8205;</a>" if poster_to_use and str(poster_to_use).upper() != "BLANK" else final_caption
                await client.edit_message_text(chat_id=edit_target["chat_id"], message_id=edit_target["message_id"], text=text_content, reply_markup=final_keyboard, disable_web_page_preview=False)
            if status_msg:
            ...
            if is_normal_photo: await client.send_photo(chat_id=MOVIE_UPDATE_CHANNEL, photo=poster_to_use, caption=final_caption, reply_markup=final_keyboard)
            else:
                text_content = f"{final_caption}\n<a href='{poster_to_use}'>&#8205;</a>" if poster_to_use and str(poster_to_use).upper() != "BLANK" else final_caption
                await client.send_message(chat_id=MOVIE_UPDATE_CHANNEL, text=text_content, reply_markup=final_keyboard, disable_web_page_preview=False)
