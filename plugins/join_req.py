...
async def join_reqs_handler(bot: Client, join_req: ChatJoinRequest):
    if not db.isActive(): return
    user = join_req.from_user
    try:
        await db.add_user(user.id, user.first_name or "Unknown", user.username or "None", join_req.date)
...
async def total_requests(bot: Client, message):
    if not db.isActive(): return await message.reply_text("⚠️ Join request DB is inactive.")
...
async def purge_requests(bot: Client, message):
    ...
    try:
        ...
        if resp.text.lower() == "y":
            ...
            await message.reply_text(f"✅ All join requests purged (`{count}` deleted).")
        else:
