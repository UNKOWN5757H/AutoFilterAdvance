...
from utils import get_settings, get_size, save_group_settings, temp, is_subscribed
...
async def silent_auto_delete(bot_message: Optional[Message], delay: int, user_or_warning_msg: Optional[Message] = None):
    ...
    if user_or_warning_msg:
        try:
            # If the bot sent the warning msg, edit it
            if getattr(user_or_warning_msg.from_user, "is_bot", False):
                await user_or_warning_msg.edit_text("<b>Hey 👋\n\nYour Request Has Been Deleted 👍\n\nIF YOU WANT THAT FILE, REQUEST AGAIN ❤️\n\nTᴇᴀᴍ: @KR_Picture</b>")
            # If the user sent a message (search text), delete it to clean the chat
            else:
                await user_or_warning_msg.delete()
        except Exception:
            try:
                await user_or_warning_msg.delete()
            except Exception:
...
def build_dynamic_keyboard(custom_buttons, static_buttons):
    ...
    for btn in custom_buttons:
        style_map = {"blue": ButtonStyle.PRIMARY, "green": ButtonStyle.SUCCESS, "red": ButtonStyle.DANGER, "gray": ButtonStyle.SECONDARY}
...
async def get_start_keyboard(user_id):
    ...
    if str(user_id) in [str(a) for a in ADMINS]:
        static_buttons.append([InlineKeyboardButton("ℹ️ 𝙷𝚎𝚕𝚙", callback_data="help"), InlineKeyboardButton("😊 𝙰𝚋𝚘𝚞𝚝", callback_data="about")])
    static_buttons.append([InlineKeyboardButton("🔗 Nᴇᴡ Rᴇʟᴇᴀꜱᴇꜱ & Oᴛᴛ Uᴘᴅᴀᴛᴇꜱ", url="https://t.me/sandalwood_kannada_moviesz", icon_custom_emoji_id=5258503720928288433, style=ButtonStyle.SUCCESS)])
    return build_dynamic_keyboard(ui.get("start_buttons", []), static_buttons)

async def get_help_keyboard():
    ui = await get_ui()
    static_buttons = [
        [InlineKeyboardButton("🖥️ UI Start", callback_data="helps_uistart"), InlineKeyboardButton("🖥️ UI Help", callback_data="helps_uihelp"), InlineKeyboardButton("🖥️ UI About", callback_data="helps_uiabout")],
    ...
    return build_dynamic_keyboard(ui.get("help_buttons", []), static_buttons)
...
async def get_about_keyboard():
    ...
    return build_dynamic_keyboard(ui.get("about_buttons", []), static_buttons)

async def transition_ui_message(client, msg_obj, text, keyboard, img=None):
    try:
        if img:
            if msg_obj.photo or msg_obj.video or msg_obj.document: await msg_obj.edit_caption(caption=text, reply_markup=keyboard, parse_mode=enums.ParseMode.HTML)
            else:
                await msg_obj.delete()
                await client.send_photo(chat_id=msg_obj.chat.id, photo=img, caption=text, reply_markup=keyboard, parse_mode=enums.ParseMode.HTML)
        else:
            if msg_obj.photo or msg_obj.video or msg_obj.document:
                await msg_obj.delete()
                await client.send_message(chat_id=msg_obj.chat.id, text=text, reply_markup=keyboard, parse_mode=enums.ParseMode.HTML)
            else:
                await msg_obj.edit_text(text=text, reply_markup=keyboard, parse_mode=enums.ParseMode.HTML)
    except MessageNotModified: pass
    except Exception as e: logger.error(f"UI Transition Error: {e}")
...
async def start(client: Client, message: Message):
    ...
    if is_help_command:
        ...
        text = text.format(mention=message.from_user.mention if message.from_user else "User", uname=bot_uname, bname=b_name)
        img = ui.get("help_img")
        keyboard = await get_help_keyboard()
        if img: return await message.reply_photo(photo=img, caption=text, reply_markup=keyboard, parse_mode=enums.ParseMode.HTML)
        else: return await message.reply_text(text=text, reply_markup=keyboard, parse_mode=enums.ParseMode.HTML)

    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        reply_markup = await get_start_keyboard(message.from_user.id if message.from_user else 0)
        text = ui.get("start_text") or script.START_TXT
        text = text.format(mention=message.from_user.mention if message.from_user else message.chat.title, uname=bot_uname, bname=b_name, plane_emoji=MESSAGE_EMOJI_PLANE, link_emoji=MESSAGE_EMOJI_LINK)
    ...
    if len(message.command) != 2:
        ...
        text = ui.get("start_text") or script.START_TXT
        caption = text.format(mention=message.from_user.mention, uname=bot_uname, bname=b_name, plane_emoji=MESSAGE_EMOJI_PLANE, link_emoji=MESSAGE_EMOJI_LINK)
    ...
    # ⚡ DEEP-LINK FILE FETCHING ENGINE
    ...
    for i, chat in enumerate(current_chats, start=start_idx + 1):
        ...
        if username: link = f"https://t.me/{username}"
            ...
            try:
                ...
                if chat_obj.username: link = f"https://t.me/{chat_obj.username}"
                elif chat_obj.invite_link: link = chat_obj.invite_link
                else: link = await client.export_chat_invite_link(chat_id)
            except Exception:
...
async def leave_channel_cmd(client: Client, message: Message):
    ...
    chat_title, tg_status, db_status = "Unknown Chat", "⚠️ Not inside chat", "⚠️ Not inside DB"
    ...
    except Exception as e: tg_status = f"⚠️ Could not leave Telegram chat: {e}"
    ...
    except Exception as e: db_status = f"❌ DB Remove Error: {e}"
    await message.reply_text(f"🎯 **Operation Complete:**\n\n<b>Name:</b> {chat_title}\n<b>ID:</b> <code>{chat_id_int}</code>\n\n<b>Telegram Status:</b> {tg_status}\n<b>Database Status:</b> {db_status}")
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    try:
        if query.data == "close_data":
            ...
            if chat_type == enums.ChatType.PRIVATE:
                ...
                if grpid is not None:
                    try:
                        chat = await client.get_chat(grpid)
                        title = chat.title
                    except Exception: return await query.answer("Join: @KR_PICTURE")
            ...
            except Exception: await query.answer("Couldn't load alert.", show_alert=True)
            ...
            files_ = await get_file_details(file_id)
            if not files_: return await query.answer("No such file exist.", show_alert=True)
            ...
            try:
                if (AUTH_CHANNEL or REQ_CHANNEL) and not await is_subscribed(client, query):
                    ...
                    m = await client.send_cached_media(chat_id=query.from_user.id, file_id=file_id, caption=f_caption, protect_content=True if ident == "filep" else False)
                    k = await client.send_message(
                        chat_id=query.from_user.id,
                        text="<b>📢 <u>Please Note</u>\n\n✅ The above file will be autodeleted in 30mins to avoid copyright issues.\n\n✅ Please forward this file to your saved messages and start downloading from there.\n\nTᴇᴀᴍ: @KR_Picture</b>"
                    )
                    await query.answer("Check PM, I have sent the files!", show_alert=True)
        ...
        # ============================================
        # ⚡ DYNAMIC UI INJECTIONS (START, HELP, ABOUT)
        # ============================================
            ...
            text = ui.get("start_text") or script.START_TXT
            text = text.format(mention=query.from_user.mention, uname=bot_uname, bname=b_name, plane_emoji=MESSAGE_EMOJI_PLANE, link_emoji=MESSAGE_EMOJI_LINK)
            img = ui.get("start_img")
            keyboard = await get_start_keyboard(query.from_user.id)
            await transition_ui_message(client, query.message, text, keyboard, img)
            ...
            text = ui.get("help_text") or script.HELP_TXT
            text = text.format(mention=query.from_user.mention, uname=bot_uname, bname=b_name)
            img = ui.get("help_img")
            keyboard = await get_help_keyboard()
            await transition_ui_message(client, query.message, text, keyboard, img)
            ...
            text = ui.get("about_text") or script.ABOUT_TXT
            text = text.format(mention=query.from_user.mention, bname=b_name)
            img = ui.get("about_img")
            keyboard = await get_about_keyboard()
            await transition_ui_message(client, query.message, text, keyboard, img)
