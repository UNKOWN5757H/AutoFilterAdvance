...
from plugins.editable import get_ui  # ⚡ DYNAMIC UI IMPORTER
...
# ============================================================
# 🎨 DYNAMIC UI BUILDER ENGINE (Prevents Indentation Errors)
# ============================================================
def build_dynamic_keyboard(custom_buttons, static_buttons):
    keyboard = []
    row = []
    
    # Render Custom MongoDB Buttons
...
async def transition_ui_message(client, msg_obj, text, keyboard, img=None):
    """Gracefully handles switching between text and photo messages safely."""
    try:
        if img:
            if msg_obj.photo or msg_obj.video or msg_obj.document:
                await msg_obj.edit_caption(caption=text, reply_markup=keyboard, parse_mode=enums.ParseMode.HTML)
            else:
...
async def start(client: Client, message: Message):
    ...
    if len(message.command) != 2:
        ...
        # Priority: DB Custom Image -> Local Config PICS -> None
...
# ============================================================
# 🧭 GLOBAL CALLBACK QUERY DISPATCHER (UI Integrated)
# ============================================================
