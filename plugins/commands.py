...
async def start(client: Client, message: Message):
    ...
    if len(message.command) != 2:
        ...
        caption = script.START_TXT.format(mention=message.from_user.mention, uname=bot_uname, bname=b_name)
        try:
...
async def leave_channel_cmd(client: Client, message: Message):
    ...
    try:
        ...
        if hasattr(db, "delete_chat"): await db.delete_chat(chat_id_int)
