...
async def start(client: Client, message: Message):
    if getattr(info, "REPAIR_MODE", False) and (not message.from_user or message.from_user.id not in info.ADMINS):
    ...
    if message.from_user and await ban_db.is_banned(message.from_user.id):
        return await message.reply_text("🚫 **You have been banned from using this bot by the ADMINS.**")

    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        await message.reply(
            script.START_TXT.format(mention=message.from_user.mention if message.from_user else message.chat.title, uname=temp.U_NAME, bname=temp.B_NAME),
    ...
    if cmd_data in ["subscribe", "help", "about"]:
        if cmd_data == "subscribe": return await ForceSub(client, message)
        return await message.reply_photo(photo=random.choice(info.PICS), caption=script.START_TXT.format(mention=message.from_user.mention, uname=temp.U_NAME, bname=temp.B_NAME), reply_markup=get_start_buttons(message.from_user.id))

    kk, file_id = cmd_data.split("_", 1) if "_" in cmd_data else (False, cmd_data)
    if not await ForceSub(client, message, file_id=file_id, mode="checksubp" if kk == "filep" else "checksub"):
        return
    ...
    if cmd_data.startswith("BATCH-"):
        ...
        if not msgs:
            ...
            try:
                with open(file_path) as f: msgs = json.load(f)
            except Exception:
        ...
        for msg in msgs:
            f_caption = info.BATCH_FILE_CAPTION.format(file_name=msg.get("title", ""), file_size=get_size(int(msg.get("size", 0))), file_caption=msg.get("caption", "")) if info.BATCH_FILE_CAPTION else msg.get("title", "")
    ...
    if not files_: return await message.reply("❌ No such file exists.")
    
    file_data = files_[0]
    f_caption = info.CUSTOM_FILE_CAPTION.format(file_name=file_data.file_name, file_size=get_size(file_data.file_size), file_caption=file_data.caption or "") if info.CUSTOM_FILE_CAPTION else file_data.file_name
    if getattr(info, "CAPTION_PLUS", None): f_caption += f"\n\n{info.CAPTION_PLUS}"
    ...
    warn_msg = await msg_sent.reply("<b><u>❗️❗️❗️IMPORTANT❗️️❗️❗️</u></b>\n\nᴛʜɪꜱ ꜰɪʟᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ɪɴ <b><u><code>30 Minutes</code></u></b>.\n<b><i>ᴘʟᴇᴀꜱᴇ ꜰᴏʀᴡᴀʀᴅ ɪᴛ ꜱᴏᴍᴇᴡʜᴇʀᴇ ᴇʟꜱᴇ.</i></b>", quote=True)
    
    if getattr(info, "FILE_AUTO_DELETE", 1800) > 0:
        asyncio.create_task(delete_after_delay(msg_sent, warn_msg, info.FILE_AUTO_DELETE))
