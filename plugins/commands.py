...
async def start(client: Client, message: Message):
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        # Pyrogram V2 Compatibility fix for DSTORE history fetching using chunked get_messages
    ...
    # ⚡ CRITICAL FIX: Pull the fully reconstructed `full_file_id`!
    db_file_id = getattr(files, "full_file_id", getattr(files, "file_id", file_id))

    if CUSTOM_FILE_CAPTION:
        try:
            # FIXED: Removed 'is' literal syntax warnings
