...
def clean_filename(name: str) -> str:
    ...
    return re.sub(
        r"\s+", " ", re.sub(r"(?i)\b(mkv|mp4|avi|webm|zip|rar)\b", "", name)
    ).strip()
...
async def advantage_spell_chok(client, msg, search_query):
    ...
    except (Forbidden, UserIsBlocked, PeerIdInvalid):
        pass
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    return await safe_send_text_filter(
                        client, chat_id, reply_text, reply_markup, reply_id
                    )
...
async def give_filter(client, message):
    ...
    if not k:
        try:
            ...
            logger.warning(f"Auto filter error: {e}")
