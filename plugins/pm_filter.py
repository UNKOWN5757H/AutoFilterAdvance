...
from pyrogram.errors import (
    ButtonUrlInvalid, ChatWriteForbidden, FloodWait, Forbidden,
    MessageIdInvalid, MessageNotModified, PeerIdInvalid, QueryIdInvalid, UserIsBlocked,
)
...
async def advantage_spell_chok(client: Client, msg: Message, search_query: str):
    ...
    if not g_s: return await _send_not_found(msg, fnf_img, fnf_txt)
    ...
    if not movielist: return await _send_not_found(msg, fnf_img, fnf_txt)
    ...
    try:
        k_msg = await msg.reply_text("<b>I couldn't find exact files. Did you mean one of these?</b>", reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.DEFAULT)
...
async def _send_not_found(msg: Message, img: Optional[str], text: str):
    k_msg = None
    try:
        if img and str(img).startswith("http"): k_msg = await msg.reply_photo(photo=img, caption=text, parse_mode=enums.ParseMode.DEFAULT)
        else: k_msg = await msg.reply_text(text=text, parse_mode=enums.ParseMode.DEFAULT)
    except Exception:
        try: k_msg = await msg.reply_text(text="<b>🚫 File not found.</b>", parse_mode=enums.ParseMode.DEFAULT)
        except Exception: pass
...
async def manual_filters(client: Client, message: Message, text: bool = False) -> bool:
    if getattr(info, "REPAIR_MODE", False):
        if not message.from_user or str(message.from_user.id) not in [str(a) for a in info.ADMINS]: return False
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            try:
                if not fileid or fileid_str in ["None", "[]", "", "False"]:
                    sent_msg = await client.send_message(chat_id=message.chat.id, text=reply_text or "", disable_web_page_preview=True, reply_markup=reply_markup, reply_to_message_id=reply_id, parse_mode=enums.ParseMode.DEFAULT)
                else:
                    sent_msg = await client.send_cached_media(chat_id=message.chat.id, file_id=fileid, caption=reply_text or "", reply_markup=reply_markup, reply_to_message_id=reply_id, parse_mode=enums.ParseMode.DEFAULT)
...
async def auto_filter(client: Client, msg: any, spoll: any = False):
    try:
        ...
        for file in files:
            ...
            if settings.get("button", False): btn.append([InlineKeyboardButton(text=f"{size_str} | {display_name}", url=url_payload)])
            else: btn.append([InlineKeyboardButton(text=f"{display_name}", url=url_payload), InlineKeyboardButton(text=f"{size_str}", url=url_payload)])
        ...
        if offset:
            ...
            btn.append([InlineKeyboardButton(text=f"1/{math.ceil(int(total_results) / 10)}", callback_data="pages"), InlineKeyboardButton(text="NEXT ➡️", callback_data=f"next_{req}_{key}_{offset}")])
        else: btn.append([InlineKeyboardButton(text="1/1", callback_data="pages")])

        m = None
        try:
            if auto_img and str(auto_img).startswith("http"): m = await message.reply_photo(photo=auto_img, caption=cap, reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.DEFAULT)
            else: m = await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.DEFAULT)
        except FloodWait as e:
            await asyncio.sleep(e.value + 1)
            try: m = await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.DEFAULT)
        ...
        delete_timer = getattr(info, "BUTTON_AUTO_DELETE", 1800)
        if delete_timer > 0 and m: asyncio.create_task(silent_auto_delete(m, delete_timer, message))
    except Exception as e: logger.error(f"Fatal error in auto_filter: {e}")
...
async def give_filter(client: Client, message: Message):
    if getattr(info, "REPAIR_MODE", False):
        if not message.from_user or str(message.from_user.id) not in [str(a) for a in info.ADMINS]: return
...
async def pagination_and_spell_handler(bot: Client, query: CallbackQuery):
    ...
    if query.data.startswith("spolling#"):
        ...
        if int(req_user) != 0 and query.from_user.id != int(req_user): return await query.answer("⚠️ This suggestion is not for you!", show_alert=True)
    ...
    try:
        if int(req) not in [query.from_user.id, 0]: return await query.answer("⚠️ That's not for you! Request your own file in the group.", show_alert=True)
    except Exception: pass
    ...
    if not n_offset: btn.append([InlineKeyboardButton("⬅️ BACK", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"Pages {current_page} / {total_pages}", callback_data="pages")])
    elif off_set is None:
        btn.insert(0, [InlineKeyboardButton(text="• Bᴀᴄᴋ Uᴘ Cʜᴀɴɴᴇʟ •", url="https://t.me/KR_Picture", icon_custom_emoji_id=5258503720928288433, style=ButtonStyle.SUCCESS)])
        btn.append([InlineKeyboardButton(f"{current_page} / {total_pages}", callback_data="pages"), InlineKeyboardButton("NEXT ➡️", callback_data=f"next_{req}_{key}_{n_offset}")])
    else: btn.append([InlineKeyboardButton("⬅️ BACK", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{current_page} / {total_pages}", callback_data="pages"), InlineKeyboardButton("NEXT ➡️", callback_data=f"next_{req}_{key}_{n_offset}")])
