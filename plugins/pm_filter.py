...
async def give_filter(client, message):
    if getattr(info, "REPAIR_MODE", False) and (not message.from_user or message.from_user.id not in info.ADMINS): return
...
async def next_page(bot, query):
    if getattr(info, "REPAIR_MODE", False) and query.from_user.id not in info.ADMINS: return
    ...
    for file in files:
        if settings.get("button", False):
            btn.append([InlineKeyboardButton(text=f"{get_size(file.file_size)} | {file.file_name}", url=f"https://t.me/{temp.U_NAME}?start=files_{file.file_id}")])
        else:
            btn.append([InlineKeyboardButton(text=f"{file.file_name}", url=f"https://t.me/{temp.U_NAME}?start=files_{file.file_id}"), InlineKeyboardButton(text=f"{get_size(file.file_size)}", url=f"https://t.me/{temp.U_NAME}?start=files_{file.file_id}")])
    ...
    if not k:
        ...
        if files:
            ...
            try:
                ...
                k_msg = await bot.send_message(chat_id=query.message.chat.id, text=f"{NOT_FOUND_TEXT}\n\n*(No photo attached due to permissions)*") if "CHAT_SEND_PHOTOS_FORBIDDEN" in str(e) else None
            except Exception: k_msg = None

            if k_msg and getattr(info, "BUTTON_AUTO_DELETE", 1800) > 0:
                asyncio.create_task(delete_message_after_delay(k_msg, info.BUTTON_AUTO_DELETE))
...
async def cb_handler(client: Client, query: CallbackQuery):
    if getattr(info, "REPAIR_MODE", False) and query.from_user.id not in info.ADMINS and query.data != "close_data": return
        ...
        if query.data == "close_data":
            ...
            if chat_type == enums.ChatType.PRIVATE:
                ...
                if grpid:
                    ...
                    return await query.answer("Not connected to any group!", show_alert=True)
            ...
            if not groupids: return await query.message.edit_text("There are no active connections!! Connect to some groups first.")
            ...
            grp_id, i, keyword = query.message.chat.id, query.data.split(":")[1], query.data.split(":")[2]
            ...
            files = files_[0]
            f_caption = CUSTOM_FILE_CAPTION.format(file_name=files.file_name or "", file_size=get_size(files.file_size) or "", file_caption=files.caption or "") if CUSTOM_FILE_CAPTION else files.file_name
            ...
            await query.answer()
            m = await client.send_cached_media(chat_id=query.from_user.id, file_id=file_id, caption=f_caption, protect_content=(ident == "checksubp"))
            ...
            if getattr(info, "FILE_AUTO_DELETE", 1800) > 0: asyncio.create_task(delete_and_notify())
            ...
            await query.answer()
            await query.message.edit_text(text=script.BANS_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "custommessages":
            await query.answer()
            await query.message.edit_text(text=script.CUSTOMMESSAGES_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "customcaption":
            await query.answer()
            await query.message.edit_text(text=script.CUSTOMCAPTION_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "delete":
            await query.answer()
            await query.message.edit_text(text=script.DELETE_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "forcesub":
            await query.answer()
            await query.message.edit_text(text=script.FORCESUB_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "filters":
            await query.answer()
            await query.message.edit_text(text=script.FILTERS_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "index":
            await query.answer()
            await query.message.edit_text(text=script.INDEX_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "promotions":
            await query.answer()
            await query.message.edit_text(text=script.PROMOTIONS_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "settings":
            await query.answer()
            await query.message.edit_text(text=script.SETTINGS_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "utilities":
            await query.answer()
            await query.message.edit_text(text=script.UTILITIES_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "connections":
            await query.answer()
            await query.message.edit_text(text=script.CONNECTIONS_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "forceadd":
            await query.answer()
            await query.message.edit_text(text=script.FORCEADD_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)

        elif query.data == "backup":
            await query.answer()
            await query.message.edit_text(text=script.BACKUP_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="help")]]), parse_mode=enums.ParseMode.HTML)
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if not message.text or message.text.startswith("/") or re.findall(r"((^\/|^,|^!|^\.|^[\U0001F600-\U000E007F]).*)", message.text) or not (2 < len(message.text) < 100):
            return
    ...
    for file in files:
        if settings.get("button"):
            ...
            btn.append([InlineKeyboardButton(text=file.file_name, url=f"https://t.me/{temp.U_NAME}?start={pre}_{file.file_id}"), InlineKeyboardButton(text=get_size(file.file_size), url=f"https://t.me/{temp.U_NAME}?start={pre}_{file.file_id}")])
...
async def advantage_spell_chok(msg):
    ...
    if not g_s:
        try:
            ...
            if getattr(info, "BUTTON_AUTO_DELETE", 1800) > 0: asyncio.create_task(delete_message_after_delay(k_msg, info.BUTTON_AUTO_DELETE))
        except Exception: pass
        return
    ...
    if not gs_parsed:
        ...
        for mv in g_s:
            if match := reg.match(mv): gs_parsed.append(match.group(1))
    ...
    if gs_parsed:
        for mov in gs_parsed:
            if imdb_s := await get_poster(mov.strip(), bulk=True): movielist += [movie.get("title") for movie in imdb_s]
        ...
        if re.search(r"( |^|[^\w])" + re.escape(keyword) + r"( |$|[^\w])", name, flags=re.IGNORECASE):
            ...
            try:
                ...
                if fileid == "None":
                    ...
                    sent_msg = await client.send_cached_media(group_id, fileid, caption=reply_text or "", reply_markup=reply_markup, reply_to_message_id=reply_id)

                if sent_msg and getattr(info, "BUTTON_AUTO_DELETE", 1800) > 0:
                    asyncio.create_task(delete_message_after_delay(sent_msg, info.BUTTON_AUTO_DELETE))
            except Forbidden as e:
                if "CHAT_SEND_PHOTOS_FORBIDDEN" in str(e) or "CHAT_SEND_MEDIA_FORBIDDEN" in str(e):
                    try:
                        sent_msg = await client.send_message(group_id, text=f"{reply_text}\n\n*(Media blocked)*" if reply_text else "*(Media blocked)*", reply_to_message_id=reply_id)
                        if sent_msg and getattr(info, "BUTTON_AUTO_DELETE", 1800) > 0: asyncio.create_task(delete_message_after_delay(sent_msg, info.BUTTON_AUTO_DELETE))
                    except Exception: pass
