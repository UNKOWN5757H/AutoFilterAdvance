...
from pyrogram.errors import (
    ...
    MessageNotModified, PeerIdInvalid, QueryIdInvalid, UserIsBlocked,
    ChatWriteForbidden
)
...
STOPWORDS = [
    ...
    "madam", "brodie", "dubbed", "guru", "chinnu", "brother", "beku",
...
def sanitize_search_query(text: str, custom_stops: Optional[List[str]] = None) -> str:
    ...
    pattern = r"\b(" + "|".join(re.escape(w) for w in combined_stops if w.strip()) + r")\b"
...
async def advantage_spell_chok(client: Client, msg: Message, search_query: str):
    ...
    btn = [[InlineKeyboardButton(text=movie[:35], callback_data=f"spolling#{user_id}#{idx}")] for idx, movie in enumerate(movielist)]
...
async def _send_not_found(msg: Message, img: Optional[str], text: str):
    ...
    try:
        if img and str(img).startswith("http"):
            k_msg = await msg.reply_photo(photo=img, caption=text, parse_mode=enums.ParseMode.DEFAULT)
        else:
            k_msg = await msg.reply_text(text=text, parse_mode=enums.ParseMode.DEFAULT)
    except Exception:
        try:
            k_msg = await msg.reply_text(text="<b>🚫 File not found.</b>", parse_mode=enums.ParseMode.DEFAULT)
        except Exception:
            pass
...
async def auto_filter(client: Client, msg: any, spoll: any = False):
    try:
        ...
        if not spoll:
            ...
            if not files:
                stripped = re.sub(r"(?i)\b(kannada|telugu|tamil|malayalam|hindi|english|dual|multi)\b", "", search).strip()
                if stripped and stripped != search:
                    files, offset, total_results = await get_search_results(stripped, max_results=10, offset=0, filter=True)
                    if files:
                        search = stripped
        ...
        for file in files:
            ...
            if settings.get("button", False):
                btn.append([InlineKeyboardButton(text=f"{size_str} | {display_name}", url=url_payload)])
            else:
                btn.append([
...
async def next_page(bot: Client, query: CallbackQuery):
    ...
    if not search:
        return await query.answer("⌛ This search expired. Please type the movie name again!", show_alert=True)
...
# ============================================================
@Client.on_callback_query(filters.regex(r"^(close_data|channels_page#.*|delallconfirm|delallcancel|groupcb.*|connectcb.*|disconnect.*|deletecb.*|backcb|alertmessage.*|file.*|checksub.*|pages|start|help|about|helps_.*|stats|rfrsh|spolling#.*)$"))
    ...
    try:
        if query.data == "close_data":
            ...
            try:
                ...
                if files:
                    await auto_filter(client, query, spoll=(selected_movie, files, offset, total_results))
                else:
            ...
            if is_owner_or_admin:
                ...
                await query.answer("You need to be Group Owner or Admin to do that!", show_alert=True)
            ...
            if (AUTH_CHANNEL or REQ_CHANNEL) and not await is_subscribed(client, query):
                return await query.answer("Search yourself in the group. Team: @KR_PICTURE", show_alert=True)
