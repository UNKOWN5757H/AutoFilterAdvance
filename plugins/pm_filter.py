...
async def send_filter_media(client, chat_id, fileid, reply_text, reply_markup, reply_id):
    """Safe media sender with FloodWait"""
    try:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    return await safe_send_text_filter(client, chat_id, reply_text, reply_markup, reply_id)
...
async def auto_filter(client, msg, spoll=False):
    ...
    for file in files:
        ...
        if settings.get("button", False):
            btn.append([InlineKeyboardButton(text=f"{get_size(file_size)} | {display_name}", url=f"https://t.me/{bot_username}?start={pre}_{file_id}")])
        else:
            btn.append([
                InlineKeyboardButton(text=f"{display_name}", url=f"https://t.me/{bot_username}?start={pre}_{file_id}"),
                InlineKeyboardButton(text=f"{get_size(file_size)}", url=f"https://t.me/{bot_username}?start={pre}_{file_id}"),
            ])
