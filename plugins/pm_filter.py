...
# ============================================================
# 🧠 NATURAL LANGUAGE QUERY CLEANER (ENGLISH + KANNADA + SLANG)
# ============================================================
STOPWORDS = [
    # Request phrases (English)
    "send", "snd", "give", "gib", "pls", "plz", "please", "need", "want",
    "upload", "uplod", "drop", "share", "find", "search", "provide", "post",
    "movie", "movies", "film", "films", "cinema", "cinemas", "full", "fullmovie",
    "download", "downlod", "link", "links", "file", "files", "print", "audio",
    "video", "ott", "hd", "hq", "bluray", "rip", "watch", "online",
    # Casual honorifics & filler words
    "bro", "bhai", "anna", "boss", "admin", "sir", "madam", "brodie", "dude",
    "macha", "machha", "guru", "chinnu", "brother",
    # Request phrases (Kannada & Tanglish)
    "beku", "bekithu", "bekittu", "bekagide", "kodi", "kodro", "kalsi", "kalsro", "kalisi",
    "haki", "haku", "hakro", "ideya", "irboda", "bidi", "madu", "yaradru",
    "chitra", "chithra", "chalanachitra", "chalanachithra", "kannadadalli",
    # Group & Channel noise
    "sandalwood", "kr_picture", "kannada_filmy_group", "telegram",
]
...
async def advantage_spell_chok(client, msg, search_query):
    ...
    if not g_s:
        try: k_msg = await msg.reply_photo(photo=pic_to_use, caption=text_to_use)
            ...
            try: k_msg = await msg.reply_text(text=text_to_use)
...
async def manual_filters(client, message, text=False):
    ...
    for keyword in reversed(sorted(keywords, key=len)):
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            button_layout = None
            if btn and btn not in ["[]", "None", "False", ""]:
                try:
                    ...
                    for row in parsed_btn:
                        ...
                        for b in row:
                            if isinstance(b, dict):
                                # Safe unpacking to prevent Pyrogram __init__ style crash
                                b_copy = b.copy()
            ...
            try:
                ...
                if not fileid or fileid_str in ["None", "[]", "", "False"]:
                    sent_msg = await client.send_message(
                        ...
                        reply_markup=reply_markup, reply_to_message_id=reply_id,
                    )
                else:
                    sent_msg = await client.send_cached_media(
                        group_id, fileid, caption=reply_text or "",
                        reply_markup=reply_markup, reply_to_message_id=reply_id,
                    )
                ...
                if sent_msg:
                    delete_timer = getattr(info, "BUTTON_AUTO_DELETE", 1800)
                    if delete_timer > 0:
                        asyncio.create_task(silent_auto_delete(sent_msg, delete_timer, message))
...
# ============================================================
# 🔍 MAIN AUTO FILTER LOGIC WITH SMART MULTI-TIER SEARCH
# ============================================================
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0, [
        InlineKeyboardButton(text="• Bᴀᴄᴋ Uᴘ Cʜᴀɴɴᴇʟ •", url="https://t.me/KR_Picture", icon_custom_emoji_id=5258503720928288433, style=ButtonStyle.SUCCESS)
    ])
...
# ============================================================
# [FIXED] Changed helps_.* to safely catch ANY help button unconditionally
        ...
        if query.data == "close_data":
            ...
            try:
                ...
                if alerts is not None:
                    ...
                    await query.answer("No alert text set for this filter.", show_alert=True)
