...
from pyrogram.errors import (
    ...
    MessageNotModified, PeerIdInvalid, QueryIdInvalid,
    RandomIdDuplicate, UserIsBlocked,
)
            ...
            for b in row:
                if isinstance(b, dict):
                    ...
                    if "style" in b_copy:
                        ...
                        if style_val == 1: b_copy["style"] = ButtonStyle.PRIMARY
                        elif style_val == 3: b_copy["style"] = ButtonStyle.SUCCESS
                        elif style_val == 4: b_copy["style"] = ButtonStyle.DANGER
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            try:
                ...
                if not fileid or fileid_str in ["None", "[]", "", "False"]:
                    sent_msg = await client.send_message(message.chat.id, reply_text, disable_web_page_preview=True, reply_markup=reply_markup, reply_to_message_id=reply_id)
                else:
                    sent_msg = await client.send_cached_media(message.chat.id, fileid, caption=reply_text or "", reply_markup=reply_markup, reply_to_message_id=reply_id)
                ...
                try:
                    if not fileid or fileid_str in ["None", "[]", "", "False"]: sent_msg = await client.send_message(message.chat.id, reply_text, disable_web_page_preview=True, reply_markup=reply_markup, reply_to_message_id=reply_id)
                    else: sent_msg = await client.send_cached_media(message.chat.id, fileid, caption=reply_text or "", reply_markup=reply_markup, reply_to_message_id=reply_id)
                    if sent_msg:
                        delete_timer = getattr(info, "BUTTON_AUTO_DELETE", 1800)
                        if delete_timer > 0: asyncio.create_task(silent_auto_delete(sent_msg, delete_timer, message))
                except Exception as e2: logger.exception(f"manual_filter retry error: {e2}")
...
async def give_filter(client, message):
    ...
    if message.from_user and await plugin_db.is_banned(message.from_user.id): return
    ...
    if not k:
        ...
        except Exception as e: logger.error(f"Auto filter error: {e}")
...
@Client.on_callback_query(filters.regex(r"^(close_data|delallconfirm|delallcancel|groupcb.*|connectcb.*|disconnect.*|deletecb.*|backcb|alertmessage.*|file.*|checksub.*|pages|start|help|about|helps_.*|stats|rfrsh)$"))
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    try:
        if query.data == "close_data": await query.message.delete()
            ...
            if groupids is None:
                try: await query.message.edit_text("There are no active connections!!")
                except (MessageIdInvalid, MessageNotModified): pass
                return
            buttons = []
            ...
            except PeerIdInvalid: await query.answer(url=f"https://t.me/{bot_username}?start={ident}_{file_id}")
            except FloodWait as e:
                await asyncio.sleep(e.value)
                try:
                    await client.send_cached_media(chat_id=query.from_user.id, file_id=file_id, caption=f_caption, protect_content=True if ident == "filep" else False)
            ...
            help_dict = {
                ...
                "helps_custommessages": ("CUSTOMMESSAGES_TXT", "💬 Custom Messages Help"),
            ...
            try: await query.message.edit_text(text=text, reply_markup=InlineKeyboardMarkup(buttons), parse_mode=enums.ParseMode.HTML)
                ...
                await query.message.edit_text(text=clean_text, reply_markup=InlineKeyboardMarkup(buttons), parse_mode=enums.ParseMode.DISABLED)
