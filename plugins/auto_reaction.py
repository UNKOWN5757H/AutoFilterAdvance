...
async def set_auto_reaction(bot: Client, message: Message):
    ...
    if target_chat.startswith("-100") or target_chat.startswith("@"):
        try:
            ...
            return await message.reply_text(
                f"❌ Could not access `{target_chat}`. Make sure I am an admin there!"
            )
    ...
    if (
        ...
        return await message.reply_text(
            "❌ You are in a Private Message. Please provide a Channel/Group ID. Example: `/setreaction @MyChannel 👍`"
        )

    if not emojis:
        return await message.reply_text(
            "❌ Please provide at least one emoji to react with."
        )
...
async def remove_auto_reaction(bot: Client, message: Message):
    ...
    if (
        ...
        return await message.reply_text(
            "❌ Please provide a Channel/Group ID. Example: `/removereaction @MyChannel`"
        )
    ...
    await message.reply_text(
        f"🗑️ **Auto-reaction has been disabled for `{target_chat_id}`.**"
    )
...
async def get_auto_reaction(bot: Client, message: Message):
    ...
    if (
        ...
        return await message.reply_text(
            "❌ Please provide a Channel/Group ID. Example: `/getreaction @MyChannel`"
        )
    ...
    if not emojis:
        await message.reply_text(
            f"ℹ️ **Auto-reaction is currently DISABLED for `{target_chat_id}`.**"
        )
    else:
        emoji_display = " ".join(emojis)
        await message.reply_text(
            f"ℹ️ **Current Auto-Reactions for `{target_chat_id}`:** {emoji_display}"
        )
...
async def auto_react_handler(bot: Client, message: Message):
    ...
    try:
        ...
        logger.debug(
            f"Failed to react to message {message.id} in {message.chat.id}: {e}"
        )
