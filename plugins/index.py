...
# ==========================================
# ⚙️ CHANNEL CONFIGURATION PARSER
# ==========================================
AUTO_INDEX_CHANNELS = []
try:
...
# ==========================================
# 🛡️ SAFE MEDIA CLONER (Prevents V2 Crashes)
# ==========================================
class MockMedia:
    pass

def wrap_media(media_obj, file_type, caption):
    """Safely clones Pyrogram read-only objects to bypass __slots__ restrictions."""
    wrapper = MockMedia()
    for attr in dir(media_obj):
        if not attr.startswith("_"):
            try: setattr(wrapper, attr, getattr(media_obj, attr))
...
async def index_files(bot, query):
    ...
    if int(from_user) not in ADMINS:
        try: await bot.send_message(int(from_user), f"Your submission for indexing `{chat}` has been accepted by our moderators and will be added soon.")
        except Exception: pass
...
async def process_index_request(bot, message, chat_id, last_msg_id):
    ...
    if message.from_user.id in ADMINS:
        ...
        try:
            ...
            temp.CURRENT = 0
            try: await ask_msg.edit("⏳ Timeout reached. Defaulting to skip `0`.")
            except Exception: pass
    ...
    async with lock:
        try:
            ...
            for i in range(0, len(message_ids), 200):
                ...
                # ⚡ FloodWait Catch & Release System
                max_retries = 3
                while max_retries > 0:
                    try:
                ...
                for message in messages:
                    ...
                    # ⚡ Safely clone object properties to bypass Python slots
                    safe_media = wrap_media(media_obj, message.media.value, message.caption)
