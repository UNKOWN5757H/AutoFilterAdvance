...
from pyrogram.errors import ChannelInvalid, ChannelPrivate, ChatAdminRequired, FloodWait, MessageNotModified
...
for attr in ["CHANNELS", "INDEX_CHANNELS"]:
    ...
    if isinstance(val, list): AUTO_INDEX_CHANNELS.extend(val)
    elif isinstance(val, (int, str)): AUTO_INDEX_CHANNELS.append(int(val))
...
async def process_index_request(bot, message, chat_id, last_msg_id):
    try:
        ...
        if k.empty: return await message.reply("Message does not exist or I lack permissions.")
    except Exception as e:
        return await message.reply(f"Error accessing chat: {e}")

    if message.from_user.id in ADMINS:
        ask_msg = await message.reply("How many messages do you want to **skip**? (Send `0` for none)\n*(60s timeout)*", quote=True)
        try:
            response = await bot.listen(chat_id=message.chat.id, filters=filters.user(message.from_user.id), timeout=60)
        ...
        btn = [[InlineKeyboardButton("Yes, Start Indexing", callback_data=f"index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}")],
               [InlineKeyboardButton("Close", callback_data="close_data")]]
        return await message.reply(f"Index Chat **{chat_id}** up to **{last_msg_id}**?\nSkipping: `{temp.CURRENT}`", reply_markup=InlineKeyboardMarkup(btn))

    # User submission
    link = chat_obj.username or str(chat_id)
    btn = [[InlineKeyboardButton("Accept", callback_data=f"index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}")],
...
async def index_command(bot, message):
    if message.reply_to_message and message.reply_to_message.forward_from_chat:
        ...
        return await process_index_request(bot, message, chat_id, message.reply_to_message.forward_from_message_id)

    if len(message.command) != 3:
        return await message.reply("**Usage:** `/index <chat_id> <last_message_id>`")
    await process_index_request(bot, message, int(message.command[1]) if message.command[1].lstrip("-").isdigit() else message.command[1], int(message.command[2]))
...
async def index_callback(bot, query):
    ...
    if action == "reject":
        ...
        return await bot.send_message(int(user), f"Your index request for `{chat}` was rejected by ADMINS.")

    if lock.locked(): return await query.answer("Another indexing process is running.", show_alert=True)
    ...
    await index_files_to_db(int(lst_msg), int(chat) if chat.lstrip('-').isdigit() else chat, query.message, bot)
    ...
    async with lock:
        try:
            ...
            for i in range(0, len(msg_ids), 200):
                ...
                try: messages = await bot.get_messages(chat, chunk)
                ...
                media_list = []
                for m in messages:
                    if m.empty or not m.media or m.media not in [enums.MessageMediaType.VIDEO, enums.MessageMediaType.AUDIO, enums.MessageMediaType.DOCUMENT]:
                        skipped += 1
                        continue
                ...
                if media_list:
                    ...
                    total += s; dups += d; errs += e

                if time.time() - last_update > 8:
                    try: await msg.edit_text(f"⚡ **Indexing...**\nSaved: `{total}`\nDuplicates: `{dups}`\nSkipped: `{skipped}`", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel", callback_data="index_cancel")]]))
