...
async def set_skip_number(bot: Client, message: Message):
    if len(message.command) > 1:
        ...
        try: skip = int(skip)
        except ValueError: return await message.reply("Skip number should be an integer.")
        temp.CURRENT = skip
...
async def index_files(bot: Client, query):
    ...
    if action == "reject":
        ...
        try: await bot.send_message(int(from_user), f"Your submission for indexing `{chat}` has been declined by our moderators.", reply_to_message_id=int(lst_msg_id))
        except Exception as e: logger.error(f"Failed to send rejection to user {from_user}: {e}")
        return
    ...
    if int(from_user) not in ADMINS:
        try: await bot.send_message(int(from_user), f"Your submission for indexing `{chat}` has been accepted by our moderators and is now processing.")
        except Exception: pass
...
async def process_index_request(bot: Client, message: Message, chat_id, last_msg_id):
    ...
    try:
        ...
        if k.empty: return await message.reply("This may be a group and I am not an admin, or the message does not exist.")
    except Exception:
    ...
    if message.from_user.id in ADMINS:
        ...
        try:
            ask_msg = await message.reply(
                ...
                f"Send a new number to change it, or send `0` to skip nothing.\n\n*(Timeout in 15s)*", quote=True
            )
            response = await bot.listen(chat_id=message.chat.id, filters=filters.user(message.from_user.id), timeout=15)
            if response and response.text:
                try: skip_count = int(response.text)
...
async def index_command(bot: Client, message: Message):
    ...
    if len(message.command) != 3:
        return await message.reply("**Usage:** `/index <chat_id_or_username> <last_message_id>`\n\n*(Note: You can also just forward a message from the channel or send a message link to start indexing!)*")

    chat_id = message.command[1]
    try: last_msg_id = int(message.command[2])
    except ValueError: return await message.reply("Last message ID must be an integer.")
...
@Client.on_message(filters.private & filters.incoming & (filters.forwarded | filters.regex(r"(https://)?(t\.me/|telegram\.me/|telegram\.dog/)(c/)?(\d+|[a-zA-Z_0-9]+)/(\d+)$")))
