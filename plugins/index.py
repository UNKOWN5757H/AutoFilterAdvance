...
# ============================================================
# 🛡️ PURE MEDIA EXTRACTOR (Bypasses Pyrogram V2 Memory Locks)
# ============================================================
class SafeMedia:
    """A pure Python object that safely stores media data for MongoDB injection."""
    def __init__(self, media_obj, file_type, caption):
        ...
        self.file_name = getattr(media_obj, "file_name", "")
...
async def auto_index_new_files(bot: Client, message: Message):
    ...
    # Extract clean data before sending to database
    safe_media = SafeMedia(media_obj, message.media.value, message.caption)
...
async def index_files(bot: Client, query):
    ...
    if query.data.startswith("idx_cancel"):
        try:
            ...
            return await query.answer("Cancelling Indexing for this channel...", show_alert=True)
        except Exception:
    ...
    if action == "reject":
        ...
        try: await bot.send_message(int(from_user), f"Your submission for indexing `{chat}` has been declined.", reply_to_message_id=int(lst_msg_id))
        except Exception: pass
        return

    if lock.locked():
        return await query.answer("Wait until the previous system-wide indexing process completes.", show_alert=True)

    msg = query.message
    await query.answer("Starting Process...⏳", show_alert=True)

    if int(from_user) not in ADMINS:
        try: await bot.send_message(int(from_user), f"Your submission for indexing `{chat}` has been accepted and is processing.")
        except Exception: pass
    ...
    try:
        await msg.edit(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel", callback_data=f"idx_cancel#{chat}")]])
        )
...
async def process_index_request(bot: Client, message: Message, chat_id, last_msg_id):
    try:
        ...
        return await message.reply("⚠️ Invalid Link/Channel or Bot is not an Admin there.")
    except Exception as e:
    ...
    try:
        ...
        if not k or getattr(k, 'empty', False):
            return await message.reply("⚠️ This may be a group and I am not an admin, or the message does not exist.")
    except Exception:
        return await message.reply("⚠️ Make sure that I am an admin in the channel (if the channel is private).")

    # If Admin triggers, instantly show Start button (NO MORE FREEZING bot.listen)
    if message.from_user and message.from_user.id in ADMINS:
        skip_count = getattr(temp, "CURRENT", 0)
        buttons = [
            [InlineKeyboardButton("Yes, Start Indexing", callback_data=f"index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}#{skip_count}")],
        ...
        return await message.reply(
            ...
            f"*(If you want to skip more messages, close this menu and use `/setskip` first)*",
    ...
    if chat_obj.username: link = f"https://t.me/{chat_obj.username}"
        ...
        except ChatAdminRequired: link = str(chat_id)

    buttons = [
        [InlineKeyboardButton("Accept Index", callback_data=f"index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}#0")],
    ...
    await message.reply("✅ Thank you for the contribution! Please wait for our moderators to verify.")
...
@Client.on_message(filters.incoming & (filters.forwarded | filters.regex(r"(https://)?(t\.me/|telegram\.me/|telegram\.dog/)(c/)?(\d+|[a-zA-Z_0-9]+)/(\d+)$")))
...
# ============================================================
async def index_files_to_db(lst_msg_id, chat, msg: Message, bot: Client, skip_number: int = 0):
    total_files = 0
    duplicate = 0
    errors = 0
    deleted = 0
        ...
        try:
            ...
            for i in range(0, len(message_ids), 200):
                ...
                for message in messages:
                    ...
                    # ⚡ Push pure data safely
                    safe_media = SafeMedia(media_obj, message.media.value, getattr(message, 'caption', ''))
                    media_to_save.append(safe_media)

                if media_to_save:
                    try:
                        # ⚡ Flexible safety check prevents unpack crashes
            ...
            logger.exception(e)
            try: await msg.edit(f"⚠️ Critical Error during Indexing: `{e}`")
            except Exception: pass
        else:
