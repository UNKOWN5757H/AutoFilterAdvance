...
async def ForceSub(bot: Client, message: Message, file_id: str = None, mode: str = None) -> bool:
    ...
    try:
        ...
        # ⚡ FIXED: Dynamically load Custom FSub Image if it was set via /setfsubimg
        b_settings = await get_bot_settings()
        custom_fsub_img = b_settings.get("fsub_img", getattr(info, "FSUB_IMG", None))
        
        try: 
            if custom_fsub_img:
                await message.reply_photo(photo=custom_fsub_img, caption="**Please join below channel to get file!**", reply_markup=InlineKeyboardMarkup(not_joined_buttons))
            else:
                await message.reply_text("Please join below channel to get file!", reply_markup=InlineKeyboardMarkup(not_joined_buttons), disable_web_page_preview=True)
