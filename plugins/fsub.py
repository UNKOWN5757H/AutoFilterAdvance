...
from pyrogram.errors import ChatAdminRequired, MessageNotModified, UserIsBlocked, UserNotParticipant
from pyrogram.types import CallbackQuery, InlineKeyboardButton, InlineKeyboardMarkup, Message

import info
from database.plugin_dbs import plugin_db as _plugin_db
# ⚡ FIXED: Imported the Join Requests DB to verify pending users
...
async def get_invite_link(bot: Client, chat_id: str) -> str:
    ...
    if channel_data.get("target"): return channel_data["target"]
    if channel_data.get("link"): return channel_data["link"]
    try:
        chat_id_int = int(str(chat_id).strip())
        chat = await bot.get_chat(chat_id_int)
        is_req = channel_data.get("type") == "req"
        invite = await bot.create_chat_invite_link(chat_id_int, creates_join_request=is_req)
...
# ⚡ FIXED: Added logic to verify users who are in the "Pending" Join Requests Queue
async def check_user_in_channel(bot: Client, channel_id: int, user_id: int, is_req: bool = False) -> bool:
    try:
    ...
    # If it's a join request channel, verify them via the DB queue
    if is_req:
        try:
            if _join_reqs_db.isActive() and await _join_reqs_db.get_user(user_id):
                return True
        except Exception as e:
            logger.error(f"Error checking join req DB: {e}")

    return False

async def ForceSub(bot: Client, message: Message, file_id: str = None, mode: str = None) -> bool:
    user = message.from_user
    if not user or str(user.id) in [str(a) for a in getattr(info, "ADMINS", [])] or not getattr(info, "IS_FSUB_ENABLED", True): return True
    ...
    try:
        for chat_id_str, data in active_fsubs.items():
            ...
            if not is_participant:
                ...
                if link:
                    ...
                    not_joined_buttons.append([InlineKeyboardButton(btn_text, url=link, icon_custom_emoji_id=5258096772776991776, style=ButtonStyle.PRIMARY)])
        ...
        not_joined_buttons.append([InlineKeyboardButton(text="✅ I've Joined", callback_data=cb_data, icon_custom_emoji_id=5258503720928288433, style=ButtonStyle.SUCCESS)])
        
        try: await message.reply_text("Please join below channel to get file!", reply_markup=InlineKeyboardMarkup(not_joined_buttons), disable_web_page_preview=True)
        except UserIsBlocked: return False
        return False
...
async def refresh_fsub_callback(bot: Client, query: CallbackQuery):
    ...
    active_fsubs = {k: v for k, v in info.FSUB_CHANNELS.items() if v.get("status") == "active"}
    if not active_fsubs: return await query.answer("Force subscribe is no longer required.", show_alert=True)
    
    not_joined_channels = False
    for chat_id_str, data in active_fsubs.items():
        is_req = data.get("type") == "req"
        is_participant = await check_user_in_channel(bot, int(chat_id_str), user_id, is_req)
        if not is_participant:
            not_joined_channels = True
            break
            
    if not_joined_channels: await query.answer("❌ You haven't joined all required channels yet!", show_alert=True)
    else:
        ...
        if file_id and file_id != "0":
            try: await bot.send_cached_media(chat_id=user_id, file_id=file_id, caption="🎉 **Thank you for joining! Here is your file:**")
            except Exception:
                try: await bot.send_message(chat_id=user_id, text="✅ **Verification complete!**\n\n⚠️ *The original file link expired. Please go back to the main group and click the file link again to get your movie!*")
                except Exception: pass
...
async def set_fsub_count(bot: Client, message: Message):
    if len(message.command) < 2: return await message.reply_text("⚙️ **Usage:** `/setfsubcount [number]`\n*Set to 0 for unlimited active channels.*")
    try:
    ...
    except ValueError: await message.reply_text("❌ **Invalid number.** Please provide a valid positive integer.")
...
async def add_dynamic_fsub(bot: Client, message: Message):
    ...
    try: chat = await bot.get_chat(int(channel_id))
    except Exception as e: return await message.reply_text(f"❌ **Failed to fetch channel.** Make sure the bot is an admin in `{channel_id}`.\n`Error: {e}`")
    ...
    try:
        ...
        if not resp or not resp.text: return await message.reply_text("❌ **Invalid response.** Please send text (y/n).")
        ...
        info.FSUB_CHANNELS[channel_id] = {"title": chat.title, "link": invite.invite_link, "target": None, "type": fsub_type, "status": status}
        await message.reply_text(f"✅ **Successfully Configured FSub!**\n\n📢 **Channel:** `{chat.title}`\n🔗 **Link:** {invite.invite_link}\n⚙️ **Type:** `{'Join Request' if is_req else 'Normal'}`\n🟢 **Status:** `{status.upper()}`\n\n*(If status is PENDING, it was queued due to your `/setfsubcount` limit)*", disable_web_page_preview=True)
    except ChatAdminRequired: await message.reply_text("❌ **The bot is not an admin in that channel or lacks 'Invite Users' rights.**")
    except asyncio.TimeoutError: await message.reply_text("⌛ **Timeout.** Setup cancelled.")
...
async def activate_fsub(bot: Client, message: Message):
    ...
    channel_id = message.command[1]
    if channel_id not in info.FSUB_CHANNELS: return await message.reply_text("❌ Channel not found in FSub database.")
    active_count = len([c for c in info.FSUB_CHANNELS.values() if c.get("status") == "active"])
    if info.FSUB_MAX_COUNT > 0 and active_count >= info.FSUB_MAX_COUNT: return await message.reply_text(f"⚠️ **Queue Full!** You already have `{active_count}` active channels (Limit: {info.FSUB_MAX_COUNT}).\nUse `/deactivatefsub` on another channel first.")
...
async def deactivate_fsub(bot: Client, message: Message):
    ...
    else: await message.reply_text("❌ Channel not found in FSub database.")
...
async def update_fsub_target(bot: Client, message: Message):
    if len(message.command) < 3: return await message.reply_text("⚙️ **Usage:** `/updatefsubtarget [channel_id] [target_link]`\n*Use 'none' to remove custom target.*")
    channel_id, target = str(message.command[1]), message.command[2]
    if channel_id not in info.FSUB_CHANNELS: return await message.reply_text(f"❌ Channel `{channel_id}` is not configured in FSub.")
    if target.lower() == "none":
        info.FSUB_CHANNELS[channel_id]["target"] = None
        await message.reply_text("✅ Custom target removed. Bot will use standard invite link.")
    else:
...
def build_fsub_list_text(title_prefix: str, filter_status: str = None) -> str:
    ...
    if filter_status: channels = [(k, v) for k, v in channels if v.get("status") == filter_status]
    if not channels: return f"❌ No {title_prefix.lower()} ForceSub channels found."
...
async def check_fsub_users(bot: Client, message: Message):
    count = await _plugin_db.get_fsub_count()
...
async def clear_fsub_users(bot: Client, message: Message):
    try:
        ...
        else: await message.reply_text("❌ Cancelled.")
    except asyncio.TimeoutError: await message.reply_text("⌛ Timeout: Operation cancelled.")
