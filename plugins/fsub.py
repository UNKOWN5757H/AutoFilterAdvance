...
def migrate_legacy_fsub():
    for attr, f_type, title in [
        ("AUTH_CHANNEL", "regular", "Main Channel"),
        ("REQ_CHANNEL", "req", "Request Channel"),
    ]:
        ch = getattr(info, attr, None)
        if ch and str(ch) not in info.FSUB_CHANNELS:
            info.FSUB_CHANNELS[str(ch)] = {
                "title": title,
                "link": None,
                "target": None,
                "type": f_type,
...
async def get_invite_link(bot: Client, chat_id: str) -> str:
    ...
    try:
        ...
        invite = await bot.create_chat_invite_link(
            int(chat_id), creates_join_request=(data.get("type") == "req")
        )
        info.FSUB_CHANNELS[chat_id].update(
            {"link": invite.invite_link, "title": chat.title}
        )
...
async def add_dynamic_fsub(bot: Client, message: Message):
    ...
    try:
        ...
        active_count = sum(
            1 for c in info.FSUB_CHANNELS.values() if c.get("status") == "active"
        )
        status = (
            "pending"
        ...
        info.FSUB_CHANNELS[channel_id] = {
            ...
            "target": None,
            "type": "req" if is_req else "regular",
            "status": status,
        }
        await message.reply_text(
            f"✅ **FSub Configured!**\n📢 **Channel:** `{chat.title}`\n🟢 **Status:** `{status.upper()}`"
...
@Client.on_message(
    filters.command(["enablefsub", "disablefsub"]) & filters.user(info.ADMINS)
)
async def toggle_fsub(bot: Client, message: Message):
    enable = message.command[0] == "enablefsub"
    info.IS_FSUB_ENABLED = enable
    await message.reply_text(
        f"{'✅ ENABLED' if enable else '❌ DISABLED'} **Force Subscribe.**"
    )
