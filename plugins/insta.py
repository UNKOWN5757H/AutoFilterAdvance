...
class InstaDatabase:
        ...
        if not self.client:
            try:
                ...
                if db_uri:
                    ...
                    self.client = AsyncIOMotorClient(db_uri, serverSelectionTimeoutMS=5000)
    ...
    async def inc_success(self, user_id, name, username, url):
        ...
        try:
            ...
            await self.db.users.update_one({"user_id": user_id}, {"$inc": {"count": 1}, "$set": {"name": name, "username": username}}, upsert=True)
...
async def enable_insta_cmd(client: Client, message: Message):
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🏢 Group Only", callback_data="instamode_group"), InlineKeyboardButton("👤 PM Only", callback_data="instamode_pm")],
        [InlineKeyboardButton("🌐 Both (Group & PM)", callback_data="instamode_both")]
    ])
    await message.reply_text("⚙️ **Where should the Instagram Downloader work?**\nChoose an option below:", reply_markup=kb)

@Client.on_callback_query(filters.regex(r"^instamode_(group|pm|both)$") & cb_admin_filter, group=1)
...
@Client.on_message(filters.command(["disableinsta", "disableintsa"]) & admin_filter, group=1)
...
@Client.on_message(filters.command(["instastats", "instastatas"]) & admin_filter, group=1)
...
async def insta_links_cmd(client: Client, message: Message):
    ...
    if log_channel:
        try: await client.send_document(chat_id=int(log_channel), document=file_path, caption=caption)
        except Exception: pass
    await msg.delete()
...
class InstaDownloader:
    ...
    def extract_url(text):
        ...
        m = re.search(r"(https?://(?:www\.)?instagram\.com/(?:p|reel|tv|stories)/[a-zA-Z0-9_\-\.]+)", text)
    ...
    def download_media(url, task_dir):
        ...
        if is_reel_or_story: return InstaDownloader._download_video(shortcode, url, task_dir)
        else: return InstaDownloader._download_photo(shortcode, url, task_dir)

    @staticmethod
    def _download_video(shortcode, url, task_dir):
        ydl_opts = {
            "quiet": True, "no_warnings": True, "outtmpl": os.path.join(task_dir, f"{shortcode}.%(ext)s"),
            "format": "bv*+ba/b", "merge_output_format": "mp4", "socket_timeout": 30, "ignoreerrors": True,
        ...
        return {"success": False, "error": "Unable to fetch video. Ensure cookies.txt is valid."}
        ...
        try:
            ...
            if nd:
                try:
                    ...
                    for carousel in re.findall(r'"edge_sidecar_to_children"[^}]*"edges":\s*\[(.*?)\]', data_str, re.DOTALL):
            ...
            if not image_urls: image_urls = [u.replace("\\u0026", "&") for u in re.findall(r'"display_url":"([^"]+)"', html) if ".mp4" not in u]
            if not image_urls: image_urls = list(set(re.findall(r'<meta\s+property="og:image"\s+content="([^"]+)"', html)))
            ...
            for i, img_url in enumerate(image_urls[:10]):
                try:
                    ...
                    r = session.get(img_url, headers={"User-Agent": "Mozilla/5.0"}, stream=True, timeout=15)
            ...
            if downloaded: return {"success": True, "file_path": downloaded[0], "file_paths": downloaded, "is_video": False, "is_multiple": len(downloaded) > 1}
    ...
    def _method_ytdlp(shortcode, task_dir):
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                for f in os.listdir(task_dir):
                    fp = os.path.join(task_dir, f)
                    if shortcode in f and not f.endswith((".mp4", ".mov", ".webm")) and os.path.getsize(fp) > 1000:
    ...
    def _method_cdn(shortcode, task_dir):
        try:
            ...
            for cdn_url in cdn_urls:
                try:
                    r = requests.get(cdn_url, headers={"User-Agent": "Mozilla/5.0"}, stream=True, timeout=15)
...
# ============================================================
@Client.on_message((filters.command("insta") | filters.regex(r"(https?://)?(www\.)?instagram\.com/(p|reel|tv|stories)/([a-zA-Z0-9_\-\.]+)")) & filters.incoming, group=1)
async def handle_instagram_link(client: Client, message: Message):
    mode = await instadb.get_settings()
    if mode == "off": return
    is_pm = message.chat.type == enums.ChatType.PRIVATE
    if mode == "group" and is_pm: return await message.reply_text("⚠️ **Instagram downloading is currently restricted to Groups only.**")
    elif mode == "pm" and not is_pm: return await message.reply_text("⚠️ **Instagram downloading is currently restricted to PMs only.**")
    ...
    if not url:
        if raw_text.startswith("/insta"): return await message.reply_text("⚠️ **Please provide a valid Instagram link.**\nExample: `/insta https://instagram.com/reel/...`")
        return
    ...
    async with download_semaphore:
        try:
            ...
            result = await loop.run_in_executor(None, InstaDownloader.download_media, url, task_dir)
            ...
            if result.get("is_multiple"):
                ...
                await message.reply_video(video=result["file_path"], supports_streaming=True, quote=True)
            else:
