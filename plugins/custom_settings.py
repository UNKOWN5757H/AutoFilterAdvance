...
async def show_stopwords(client, message):
    ...
    if not current_words:
        return await message.reply_text("ℹ️ **No custom stopwords have been added yet.**")
    
    words_str = ", ".join(current_words)
    await message.reply_text(f"🛑 **Current Custom Stopwords:**\n\n`{words_str}`\n\n**Total:** `{len(current_words)}`")
...
async def rem_stopwords(client, message):
    ...
    if not current_words:
        return await message.reply_text("ℹ️ **There are no custom stopwords to remove.**")
...
async def rem_all_stopwords(client, message):
    ...
    await message.reply_text("🗑️ **All custom stopwords have been successfully removed.**")
