...
async def bot_stats_cmd(bot: Client, message: Message):
    status_msg = await message.reply_text(
        "⏳ **Checking active users in database... This might take a while depending on your DB size.**"
    )
        ...
        if processed % 100 == 0:
            try:
                await status_msg.edit_text(
                    f"⏳ **Checking users:** `{processed} / {total_users}`"
                )
            except:
                pass
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🔥 YES, NUKE FILES", callback_data="nuke_files"
                    )
                ],
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🔥 YES, NUKE USERS", callback_data="nuke_users"
                    )
                ],
                [InlineKeyboardButton("❌ Cancel", callback_data="close_data")],
            ]
