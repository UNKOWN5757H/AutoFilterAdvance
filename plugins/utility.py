...
@Client.on_message(filters.command("logs") & admin_filter & (filters.private | filters.group), group=3)
async def get_logs_cmd(bot: Client, message: Message):
    log_file = "TelegramBot.log"
    if not os.path.exists(log_file): return await message.reply_text("⚠️ **Log file not found!**")
    try: await message.reply_document(document=log_file, caption="📜 **Here are the latest bot logs.**")
    except Exception as e: await message.reply_text(f"❌ **Failed to send logs:**\n`{e}`")

@Client.on_message(filters.command("server") & admin_filter & (filters.private | filters.group), group=3)
...
@Client.on_message(filters.command("restart") & admin_filter & (filters.private | filters.group), group=3)
async def restart_bot_cmd(bot: Client, message: Message):
    await message.reply_text("⚠️ **Are you sure you want to restart the bot?**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Confirm Restart", callback_data="util_restart")]]))
...
@Client.on_message(filters.command("stats") & admin_filter & (filters.private | filters.group), group=3)
async def bot_stats_cmd(bot: Client, message: Message):
    ...
    total_files = await _Media.count_documents()
...
@Client.on_message(filters.command("cleanusers") & admin_filter & (filters.private | filters.group), group=3)
async def clean_users_cmd(bot: Client, message: Message):
    ...
    if not isinstance(users, list):
        try: users = await users.to_list(length=None)
        except AttributeError: users = list(users)
    active, blocked = 0, 0
...
@Client.on_message(filters.command("total") & admin_filter & (filters.private | filters.group), group=3)
...
@Client.on_message(filters.command("clearfiles") & admin_filter & (filters.private | filters.group), group=3)
async def clear_files_cmd(bot: Client, message: Message):
    await message.reply_text("⚠️ **WARNING!** ⚠️\nDelete **ALL** files indexed in your database?", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔥 YES, DELETE ALL FILES", callback_data="nuke_files")], [InlineKeyboardButton("❌ Cancel", callback_data="close_data")]]))

@Client.on_message(filters.command("clearusers") & admin_filter & (filters.private | filters.group), group=3)
async def clear_users_cmd(bot: Client, message: Message):
    await message.reply_text("⚠️ **WARNING!** ⚠️\nDelete **ALL** users from your database?", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔥 YES, DELETE ALL USERS", callback_data="nuke_users")], [InlineKeyboardButton("❌ Cancel", callback_data="close_data")]]))

@Client.on_message(filters.command("clearfsubusers") & admin_filter & (filters.private | filters.group), group=3)
async def clear_fsub_cmd(bot: Client, message: Message):
    await message.reply_text("⚠️ **WARNING!** ⚠️\nClear the Force Sub DB?", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔥 YES, CLEAR FSUB", callback_data="nuke_fsub")], [InlineKeyboardButton("❌ Cancel", callback_data="close_data")]]))
