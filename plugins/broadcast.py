...
from pyrogram.errors import FloodWait, InputUserDeactivated, PeerIdInvalid, UserIsBlocked, ChatWriteForbidden
...
async def user_broadcast(bot: Client, message):
    ...
    for idx, user in enumerate(users, 1):
        status = await send_and_schedule_delete(bot, int(user["id"]), message.reply_to_message)
        ...
        if idx % 20 == 0:
            try: await status_msg.edit_text(f"📢 **Progress:** {idx}/{len(users)}\n✅ Sent: {success} | 🚫 Blocked: {blocked}")
...
async def group_broadcast(bot: Client, message):
    ...
    status_msg = await message.reply_text(f"🚀 **Broadcasting to {len(chats)} groups...**")
    success, left, failed = 0, 0, 0

    for idx, chat in enumerate(chats, 1):
        status = await send_and_schedule_delete(bot, int(chat["id"]), message.reply_to_message)
        if status == 200: success += 1
        elif status == 400: left += 1
        else: failed += 1

        if idx % 20 == 0:
            try: await status_msg.edit_text(f"🏘️ **Progress:** {idx}/{len(chats)}\n✅ Sent: {success} | 🚷 Removed: {left}")
            except Exception: pass
