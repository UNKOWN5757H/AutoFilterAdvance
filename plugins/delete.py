...
from database.ia_filterdb import Media as _Media, unpack_new_file_id
...
async def delete_single_file(bot: Client, message: Message):
    try:
        ...
        if not media:
            return await status_msg.edit_text("❌ **This is not a supported file format.**")

        file_id, _ = unpack_new_file_id(media.file_id)

        res = await _Media.collection.delete_one({"_id": file_id})
        if res.deleted_count:
            return await status_msg.edit_text("✅ **File successfully deleted from database.**")
        ...
        if res.deleted_count:
            await status_msg.edit_text(f"✅ **Deleted {res.deleted_count} duplicate files from database.**")
        else:
...
async def delete_multiple_files(bot: Client, message: Message):
    if len(message.command) < 2: return await message.reply_text("⚙️ **Usage:** `/delmulti <keyword/name>`")
    keyword = message.text.split(None, 1)[1]
    status_msg = await message.reply_text(f"⏳ **Searching and deleting files matching:** `{keyword}`...")
    ...
    try:
        ...
        if result.deleted_count > 0:
            await status_msg.edit_text(f"✅ **Successfully deleted `{result.deleted_count}` files matching:** `{keyword}`")
        else:
...
async def set_autodelete_timer(bot: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(f"⚙️ **Usage:** `/autodelete [seconds]`\n\n⏱ **Current File Auto-Delete:** `{info.FILE_AUTO_DELETE}` seconds.")
    try:
        seconds = int(message.command[1])
        if seconds < 0: raise ValueError
        info.FILE_AUTO_DELETE = seconds

        if seconds == 0: await message.reply_text("✅ **File Auto-Delete has been DISABLED (0 seconds).**")
        else: await message.reply_text(f"✅ **File Auto-Delete successfully set to:** `{seconds}` seconds.")
    except ValueError:
        await message.reply_text("❌ **Invalid duration!** Please provide a valid positive integer (seconds).")


@Client.on_message(filters.command("buttondel") & admin_filter)
async def set_buttondel_timer(bot: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(f"⚙️ **Usage:** `/buttondel [seconds]`\n\n⏱ **Current Button Auto-Delete:** `{info.BUTTON_AUTO_DELETE}` seconds.")
    try:
        seconds = int(message.command[1])
        if seconds < 0: raise ValueError
        info.BUTTON_AUTO_DELETE = seconds

        if seconds == 0: await message.reply_text("✅ **Group Button Auto-Delete has been DISABLED (0 seconds).**")
        else: await message.reply_text(f"✅ **Group Button Auto-Delete successfully set to:** `{seconds}` seconds.")
    except ValueError:
        await message.reply_text("❌ **Invalid duration!** Please provide a valid positive integer (seconds).")
