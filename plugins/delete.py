...
async def delete_single_file(bot: Client, message: Message):
    if len(message.command) == 2:
        ...
        return await message.reply_text(
            "✅ **File deleted.**" if res.deleted_count else "⚠️ **File not found.**"
        )

    reply = message.reply_to_message
    if not (reply and reply.media):
        return await message.reply_text(
            "⚙️ **Usage:** `/delete <file_id>` or reply to a file."
        )

    media = getattr(reply, reply.media.value, None)
    if not media:
        return await message.reply_text("❌ **Unsupported format.**")

    file_id, _ = unpack_new_file_id(media.file_id)
    res = await Media.collection.delete_one({"_id": file_id})
    if res.deleted_count:
        return await message.reply_text("✅ **File deleted.**")

    # Fallback to name match
    file_name = re.sub(r"(_|\-|\.|\+)", " ", str(getattr(media, "file_name", "")))
    res = await Media.collection.delete_many(
        {"file_name": file_name, "file_size": getattr(media, "file_size", 0)}
    )
    await message.reply_text(
        f"✅ **Deleted {res.deleted_count} duplicate files.**"
        if res.deleted_count
        else "⚠️ **File not found.**"
    )
...
async def delete_multiple_files(bot: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text("⚙️ **Usage:** `/delmulti <keyword>`")
    keyword = message.text.split(None, 1)[1]
    res = await Media.collection.delete_many(
        {"file_name": {"$regex": keyword, "$options": "i"}}
    )
    await message.reply_text(
        f"✅ **Deleted `{res.deleted_count}` files matching:** `{keyword}`"
        if res.deleted_count
        else f"⚠️ **No files found matching:** `{keyword}`"
    )
...
async def set_autodelete_timer(bot: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            f"⏱ **Current File Auto-Delete:** `{info.FILE_AUTO_DELETE}`s"
        )
    try:
        info.FILE_AUTO_DELETE = int(message.command[1])
        await message.reply_text(
            f"✅ **File Auto-Delete set to:** `{info.FILE_AUTO_DELETE}`s"
        )
    except ValueError:
...
async def set_buttondel_timer(bot: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            f"⏱ **Current Button Auto-Delete:** `{info.BUTTON_AUTO_DELETE}`s"
        )
    try:
        info.BUTTON_AUTO_DELETE = int(message.command[1])
        await message.reply_text(
            f"✅ **Button Auto-Delete set to:** `{info.BUTTON_AUTO_DELETE}`s"
        )
    except ValueError:
        await message.reply_text("❌ **Invalid duration!**")
