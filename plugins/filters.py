...
async def add_filter_cmd(client: Client, message: Message):
    ...
    text, btn = parse_markdown_buttons(
...
async def del_filter_cmd(client: Client, message: Message):
    ...
    await delete_filter(
        message, message.text.split(None, 1)[1].lower(), message.chat.id
    )
...
async def list_filters_cmd(client: Client, message: Message):
    ...
    if not keywords:
        return await message.reply_text("⚠️ **No filters found.**")
