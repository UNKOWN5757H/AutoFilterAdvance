            ...
            try:
                if not fileid or fileid_str in ["None", "[]", "", "False"]:
                    try:
                        ...
                        sent_msg = await client.send_message(message.chat.id, reply_text or "", disable_web_page_preview=True, reply_markup=reply_markup)
                else:
                    try:
                        ...
                        try:
                            sent_msg = await client.send_photo(message.chat.id, photo=fileid, caption=reply_text or "", reply_markup=reply_markup)
                        except Exception:
                            try:
                                sent_msg = await client.send_document(message.chat.id, document=fileid, caption=reply_text or "", reply_markup=reply_markup)
                            except Exception:
                                sent_msg = await client.send_message(message.chat.id, text=reply_text or "", reply_markup=reply_markup)
            except Exception as e:
                logger.error(f"Error sending filter: {e}")
