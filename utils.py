...
from pyrogram.errors import FloodWait, InputUserDeactivated, PeerIdInvalid, UserIsBlocked, UserNotParticipant
...
def _fetch_imdb_data(query, bulk=False, id=False, file=None):
    try:
        if not id:
            ...
            filtered = [k for k in movieid if str(k.get("year")) == str(year)] if year else movieid
            if not filtered: filtered = movieid
            
            movieid_filtered = [k for k in filtered if k.get("kind") in ["movie", "tv series"]]
            if not movieid_filtered: movieid_filtered = filtered
            
            if bulk: return movieid_filtered
        ...
        return {
            ...
            "seasons": movie.get("number of seasons"), "box_office": movie.get("box office"), "localized_title": movie.get("localized title"),
            "kind": movie.get("kind"), "imdb_id": f"tt{movie.get('imdbID')}", "cast": list_to_str(movie.get("cast")),
            "runtime": list_to_str(movie.get("runtimes")), "countries": list_to_str(movie.get("countries")), "certificates": list_to_str(movie.get("certificates")),
            "languages": list_to_str(movie.get("languages")), "director": list_to_str(movie.get("director")), "writer": list_to_str(movie.get("writer")),
            "producer": list_to_str(movie.get("producer")), "composer": list_to_str(movie.get("composer")), "cinematographer": list_to_str(movie.get("cinematographer")),
            "music_team": list_to_str(movie.get("music department")), "distributors": list_to_str(movie.get("distributors")), "release_date": date,
            "year": movie.get("year"), "genres": list_to_str(movie.get("genres")), "poster": movie.get("full-size cover url"),
            "plot": plot, "rating": str(movie.get("rating", "N/A")), "url": f"https://www.imdb.com/title/tt{movieid}",
        }
    except Exception as e:
...
def extract_user(message: Message) -> Union[int, str]:
    if message.reply_to_message: return (message.reply_to_message.from_user.id, message.reply_to_message.from_user.first_name)
        ...
        try: return (int(message.command[1]), str(message.command[1]))
        except ValueError: return (message.command[1], str(message.command[1]))
...
def remove_escapes(text: str) -> str:
    ...
    for char in text:
        if is_escaped: res += char; is_escaped = False
