...
class PluginDatabase:
    ...
    async def add_promo(self, text: str, url: str):
        await self.promo_col.update_one({"url": url}, {"$set": {"text": text, "url": url}}, upsert=True)
    ...
    async def ban_user(self, user_id: int):
        await self.ban_col.update_one({"_id": user_id}, {"$set": {"_id": user_id}}, upsert=True)
    ...
    async def add_fsub_user(self, user_id: int):
        await self.fsub_col.update_one({"_id": user_id}, {"$set": {"_id": user_id}}, upsert=True)
    ...
    async def set_reaction_status(self, status: bool):
        await self.react_col.update_one({"_id": "global_status"}, {"$set": {"is_enabled": status}}, upsert=True)

    # ================= FORCE ADD =================
    async def set_fa_settings(self, chat_id: int, limit: int, mode: str):
        await self.fa_col.update_one({"_id": f"settings_{chat_id}"}, {"$set": {"limit": limit, "mode": mode}}, upsert=True)
    ...
    async def track_fa_new_user(self, chat_id: int, user_id: int):
        await self.fa_col.update_one({"_id": f"new_users_{chat_id}"}, {"$addToSet": {"users": user_id}}, upsert=True)

    async def is_fa_new_user(self, chat_id: int, user_id: int) -> bool:
        doc = await self.fa_col.find_one({"_id": f"new_users_{chat_id}", "users": user_id})
        return bool(doc)
    ...
    async def increment_fa_adds(self, chat_id: int, user_id: int, count: int):
        ...
        week_ago = now - 604800
        await self.fa_col.update_one({"_id": key}, {"$pull": {"history": {"$lt": week_ago}}})
    ...
    async def reset_fa_daily_adds(self, chat_id: int):
        await self.fa_col.update_many({"_id": {"$regex": f"^adds_{chat_id}_"}}, {"$set": {"history": []}})
    ...
    async def get_fa_top_adds(self, chat_id: int, time_limit: int = None) -> list:
        ...
        async for doc in cursor:
            ...
            if time_limit is None:
                ...
                score = len([ts for ts in doc.get("history", []) if ts >= (now - time_limit)])
            if score > 0:
