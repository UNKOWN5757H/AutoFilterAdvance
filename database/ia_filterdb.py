...
def add_to_vocab(file_name):
    ...
    if clean: KNOWN_TITLES.add(clean)
...
class Media(Document):
    ...
    class Meta:
        indexes = [{"key": [("file_name", "text"), ("caption", "text")], "name": "media_text_index", "weights": {"file_name": 10, "caption": 2}}]
...
async def get_search_results(query_text, file_type=None, max_results=10, offset=0, filter=False, is_autocorrect=False):
    ...
    if parsed.get("season") is not None: conditions.append({"file_name": {"$regex": rf"(s0?{parsed['season']}\b|season\s*0?{parsed['season']}\b)", "$options": "i"}})
    if parsed.get("episode") is not None: conditions.append({"file_name": {"$regex": rf"(e0?{parsed['episode']}\b|ep\s*0?{parsed['episode']}\b|episode\s*0?{parsed['episode']}\b)", "$options": "i"}})

    for y in parsed.get("years", []): conditions.append({"file_name": {"$regex": rf"\b{y}\b", "$options": "i"}})
    for q in parsed.get("qualities", []): conditions.append({"file_name": {"$regex": rf"\b{q}\b", "$options": "i"}})
    for l in parsed.get("languages", []):
        conditions.append({"$or": [{"file_name": {"$regex": rf"\b{l}\b", "$options": "i"}}, {"caption": {"$regex": rf"\b{l}\b", "$options": "i"}}]} if USE_CAPTION_FILTER else {"file_name": {"$regex": rf"\b{l}\b", "$options": "i"}})

    if not conditions: conditions.append({"file_name": {"$regex": re.escape(query_text).replace(r"\ ", r".*"), "$options": "i"}})

    mongo_query = {"$and": conditions} if len(conditions) > 1 else conditions[0]
    if file_type: mongo_query = {"$and": [mongo_query, {"file_type": file_type}]}
    ...
    if total_results == 0 and not is_autocorrect and process and KNOWN_TITLES:
        if best_match := process.extractOne(query_text, KNOWN_TITLES):
            if 75 <= best_match[1] <= 100:
                ...
                if corrected[2] > 0: search_cache.set(cache_key, corrected)
                return corrected
    ...
    if total_results > 0: search_cache.set(cache_key, result)
    return result
    ...
    for media in media_list:
        try:
            ...
            documents.append({
                ...
                "file_size": getattr(media, "file_size", 0), "file_type": getattr(media, "file_type", None),
                "mime_type": getattr(media, "mime_type", None), "caption": str(getattr(media, "caption", None)) if getattr(media, "caption", None) else None
            })
        except Exception: pass

    if not documents: return 0, 0, len(media_list)
    try:
...
async def get_file_details(query):
    return [SafeMediaWrapper(f) for f in await db[COLLECTION_NAME].find({"_id": query}).to_list(length=1)]

def unpack_new_file_id(new_file_id):
    decoded = FileId.decode(new_file_id)
    file_id = base64.urlsafe_b64encode(pack("<iiqq", int(getattr(decoded.file_type, "value", decoded.file_type)), getattr(decoded, "dc_id", 0), getattr(decoded, "media_id", None) or getattr(decoded, "id", 0), getattr(decoded, "access_hash", 0))).decode().rstrip("=")
